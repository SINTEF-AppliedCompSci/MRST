...
function jac = divJac(jac, N, nc, nf, sortIx, C, prelim, useMex)
    ...
    elseif jac.isZero
        ...
        if useMex && (isempty(jac.parentSubset) || all(jac.parentSubset == (1:jac.dim(1))'))
            jac = mexDiscreteDivergenceJac([], jac.diagonals, N, prelim.facePos, prelim.faces, prelim.cells, prelim.cellIndex);
        else
    ...
    elseif jac.isZero
        ...
        if useMex && (isempty(jac.parentSubset) || (numel(jac.parentSubset) == jac.dim(1)) && all(jac.parentSubset == (1:jac.dim(1))'))
            if isa(acc, 'DiagonalJacobian')
                ...
                jac = mexDiscreteDivergenceJac(acc.diagonals, jac.diagonals, N, prelim.facePos, prelim.faces, prelim.cells, prelim.cellIndex);
            else
                jac = acc + mexDiscreteDivergenceJac([], jac.diagonals, N, prelim.facePos, prelim.faces, prelim.cells, prelim.cellIndex);
            end
        else
