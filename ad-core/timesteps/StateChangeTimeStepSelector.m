    ...
    methods
        ...
        function dt = computeTimestep(selector, dt, dt_prev, model, solver, state_prev, state_curr)
            ...
            for i = 1:numel(selector.targetProps)
                ...
                assert(numel(curr) == numel(prev), ['Mismatch in property size: ', fn]);
                for jj = 1:numel(curr)
                    c = curr{jj};
                    p = prev{jj};
                    if iscell(c)
                    ...
                    dispif(selector.verbose, 'Property ''%s'': Rel change %1.2f, abs change: %1.2f -> adjustment of %1.2f\n', ...
                        fn, maxRel, maxAbs, min(f_rel, f_abs));
                end
                dt = min(dt, dt_next);
            end
