function [wells, states] = runJutulOnDaemon(state0, model, schedule, varargin)
    ...
    pth = writeJutulInput(state0, model, schedule, opt.name, 'printcmd', false);
    ...
    cmd = sprintf('julia %s --startup-file=no --color=no -e "using DaemonMode; runargs()" %s', opt.project, cmd_pth);
    id = system(cmd);
    if id == 1
