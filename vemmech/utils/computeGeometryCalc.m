function G = computeGeometryCalc(G, varargin)
% HACK to calculate weight needed for VEM/finite element type of
% calculations. Basicaly a copy of computeGeometry
...
%} 
    ...
    if G.griddim == 3, 
        ...
        %%
        % Divide each face into sub-triangles all having one node at
        % pCenter = sum(nodes) /#nodes. Compute area-weighted normals, 
        % and add to obtain approximateion of face-normals. Compute the resulting areas
        % and centroids.
        ...
        weights = struct('face_nodes', subAreas);
        ...
        % all having one node at cCenter = sum(faceCentroids) / #faceCentroids.
        ...
        lastInx = 0;
        weights.cell_node = nan(size(G.cells.nodes));
        for c = 1 : numC, 
