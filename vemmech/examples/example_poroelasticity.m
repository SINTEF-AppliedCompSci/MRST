...
% This example sets up a poro-elastic problem which mimics a slice of
% overburden, with an infinite horizontal well in an aquifer at the bottom of
% the domain. The poro-elastic equations are set up together explicitely. At the
% end we simulate the same case by using the poro-elastic solver.
%
...
% Use a TPFA solver for weakly compressible. From the output we retrieve the
% operators for the fluid.
dt = day; % time step.
...
while t < end_time
    t = t + dt;
    % We update the right-hand side for the fluid (divergence term comes from
    % mechanics).
