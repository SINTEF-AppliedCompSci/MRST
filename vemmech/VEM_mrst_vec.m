...
%}
   ...
   % opt = struct('local_assembly', false, 'has_weight', false, ...
   %              'alpha_type', true, ...
   %              'corigon', false, ...
   %              'blocksize', 1000);
   ...
   if nargout > 1
      %warning('nblocks is put to 1 for extra output'); 
      nblocks = 1;
   else
   ...
   % initialize a zero matrix to be added into in case of block assembly
   ...
   for iblocks = 1:numel(blocks)
      ...
      % dimension of Voigt matrix and therefore also dimensi on of the local linear space
      if(G.griddim == 2)
         nlin = 3;        
      else
         nlin = 6; % G.grddim == 3
      ...
      %% define projection operators in nodal basis
      ...
      trD = sum(D_all(cells, indd), 2); %trace D
      ...
      if isempty(opt.S)
         % calculate inner product by consistency and orthogonal part
         DNC=diag(NC'*NC);
         if(~opt.experimental_scaling)
            ...
            % trDNCn=sum(1./trDNCn(1:G.griddim,:),1)';% maybe use all
            trDNCn=sum(1./trDNCn, 1)';% use all
            alpha = rldecode((1/nlin^2)*trD.*vol.*trDNCn, nlc*G.griddim); % not properly scaled
         end
