...
l2err1 = l2Error(G, sol1, gD, 1);
l2err1 = sqrt(sum(l2err1(trustCells)))/gDl2;
err1   = sol1.nodeValues-uChi;
err1   = norm(err1(trustNodes),2)/gD2;

l2err2 = l2Error(G, sol2, gD, 1);
l2err2 = sqrt(sum(l2err2(trustCells)))/gDl2;
err2   = sol2.nodeValues-uChi;
err2   = norm(err2(trustNodes),2)/gD2;


l2err3 = l2Error(G, sol3, gD, 1);
l2err3 = sqrt(sum(l2err3(trustCells)))/gDl2;
err3   = sol3.nodeValues-uChi;
err3   = norm(err3(trustNodes),2)/gD2;


l2err4 = l2Error(G, sol4, gD, 1);
l2err4 = sqrt(sum(l2err4(trustCells)))/gDl2;
err4   = sol4.nodeValues-uChi;
err4   = norm(err4(trustNodes),2)/gD2;
...
%    err = sol.nodeValues - uChi;    
%    errVec(i) = norm(max(abs(err(trustNodes))),2);
% 
%     
% end
% 
...
% 
% set(gcf, 'defaulttextinterpreter', 'latex');
% xlabel('$w$'); ylabel('Scaled error')
% 
% h = legend('Euclidean norm', '$L^2$-norm');
...
% print(gcf, '-dpdf', '../../tex/thesis/fig/wStability.pdf');
