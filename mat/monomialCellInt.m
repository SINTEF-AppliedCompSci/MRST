...
intDx = bsxfun(@times, ...
              (int_mx(Xx(1:nNx/2,:))  + int_mx(Xx(nNx/2+1:nNx,:)))/12 + ...
              (int_mx(Xqx(1:nNx/2,:)) + int_mx(Xqx(nNx/2+1:nNx,:)))*5/12, ...
               normProdx);
intDy = bsxfun(@times,(int_my(Xy(1:nNy/2,:))  + int_my(Xy(nNy/2+1:nNy,:)))/12 + ...
                     (int_my(Xqy(1:nNy/2,:)) + int_my(Xqy(nNy/2+1:nNy,:)))*5/12, ...
                     normPrody);
                            %   Assemble all integrals
I = zeros(nE,10); I(~eYZ,:) = intDx; I(eYZ,:) = intDy;
                            %   Sum for all edges of each face.

I = mat2cell(I, cellFaceNodes, 10);
I = cellfun(@(X) sum(X,1), I, 'UniformOutput', false);
I = bsxfun(@times, cell2mat(I), hK.^2);
