function total_error = objectivefun_sync_multi_obj_heterogeneous(x, ...
    model_flooding, model_cent)
%
% DESCRIPTION: defines the objective function including the simultaneous
%              case, and heterogeneity for multi objective optimization
%              using genetic algorithm
%
% SYNOPSIS:
%   total_error = objectivefun_sync_multi_obj_heterogeneous(x, ...
%     model_flooding, model_cent)
%
% PARAMETERS:
...
try
    model_flooding = run_history_match_heterogeneous(x, model_flooding, 'ss');
    ...
    if strcmp(obj_fun_type,'Simultaneous')
        model_cent = run_history_match_heterogeneous(x, model_cent, 'cent');
