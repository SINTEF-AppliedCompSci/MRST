function parameter_struct = modified_corey_masalmeh_kr_handle(model, x, row_names)
%
...
%
% SYNOPSIS:
%   parameter_struct = modified_corey_masalmeh_kr_handle(model, x, row_names)
%
% PARAMETERS:
...
% handle cW
cW_mask = strcmpi(row_names, 'cW');
if any(cW_mask)
    parameter_struct.cW = x(cW_mask);
else
    if isfield(kr_input_struct, 'cW')
        parameter_struct.cW = kr_input_struct.cW;
    else
        parameter_struct.cW = 0.01;
    end
end

% handle cNW
cNW_mask = or(strcmpi(row_names, 'cNW'),strcmpi(row_names, 'co'));
if any(cNW_mask)
    parameter_struct.cNW = x(cNW_mask);
else
    if isfield(kr_input_struct, 'cNW')
        parameter_struct.cNW = kr_input_struct.cNW;
    else
        parameter_struct.cNW = 0.01;
    end
end
