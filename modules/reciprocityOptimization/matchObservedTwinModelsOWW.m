function obj = matchObservedTwinModelsOWW(model_1, states_1, schedule_1, model_2, states_2, schedule_2,observed, bd_cells,bd_faces,well_cells,well_data,varargin)
...
F1 = vertcat(state_1.wellSol.ComponentTotalFlux);
F2 = vertcat(state_2.wellSol.ComponentTotalFlux);
F1= sum(F1,2);
F2= sum(F2,2);
aa=[schedule_1.control(1).W.cells]';
...
if opt.mismatchSum
    ...
    mm ={fac.*abs(wp.*(pressure_1{1}(aa)-pressure_2{1}(aa)).*(F1(:,1)-F2(:,1))),...
        fac*(wp.*matchCases.*(bhp_2 - bhp_1)).^2, ...
        fac*(wp.*matchCases.*(qWs_2 - qWs_obs)).^2, ...
        fac*(ww.*matchCases.*(qWs_1 - qWs_obs)).^2, ...
        fac*(wo.*matchCases.*(qOs_1 - qOs_obs)).^2, ...
        fac*(wp.*matchCases.*(qOs_2 - qOs_obs)).^2,...
         0.0.*interWellEnergyError, ...
         10.01.*L2NormPressureWellsError, ...
         0.0*H1NormPressureError, ...
        0.00*energyNormError};
    mmm  = {fac*(ww.*matchCases.*(qWs_2 - qWs_1)).^2, ...
        fac*(wo.*matchCases.*(qOs_2 - qOs_1)).^2, ...
        fac*(wp.*matchCases.*(bhp_2 - bhp_1)).^2, ...
        fac*(ww.*matchCases.*(qWs_1 - qWs_obs)).^2, ...
        fac*(wo.*matchCases.*(qOs_1 - qOs_obs)).^2, ...
        fac*(wp.*matchCases.*(bhp_2 - bhp_obs)).^2,...
         0.0.*interWellEnergyError, ...
         10.01.*L2NormPressureWellsError, ...
         0.0*H1NormPressureError, ...
        0.00*energyNormError};
