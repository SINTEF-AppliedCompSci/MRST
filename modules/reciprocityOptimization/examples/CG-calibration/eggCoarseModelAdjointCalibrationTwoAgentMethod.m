...
schedule.step.val     = schedule.step.val(1:30)./4;
schedule.step.control = schedule.step.control(1:30);
...
blockIx = compressPartition(blockIx);
% Perform a simple upscaling to obtain a coarse model
modelCoarse_new = modelCoarse;%upscaleModelTPFA(modelRef_new, blockIx);
...
% modelCoarse_new.toleranceCNV = 1e-8;  % tighter tolerance to improve gradient accuracy
% modelCoarse_new.FacilityModel.toleranceWellBHP = 40000;
% modelCoarse_new.FacilityModel.toleranceWellRate = 1.0e-9;
% modelCoarse_new.FacilityModel.nonlinearTolerance = 1.0e-8;
% modelCoarse_new.FacilityModel.toleranceWellMS = 1.0e-8;
...
scheduleCoarse_new = flipWellControlsAndAddBC(scheduleCoarse, statesRef, []);
...
setup_init_new = struct('model', modelCoarse, 'schedule', scheduleCoarse_new, 'state0', stateCoarse0);
...
config_new = {...
    ...
    'conntrans',        1,        'log',                  [],     [1e-3, 1e2]      
    'transmissibility', 1,        'log',                  [],     [1e-3  1e2] 
    'swl',              1,     'linear',             [0, .3],              []
    'swcr',             1,     'linear',             [0, .4],              []
    'swu',              1,     'linear',             [.7, 1],              []
    'sowcr',            1,     'linear',             [0, .4],              []
...
val = applyFunction(@(p)p.getParameter(setup_init), parameters);
% pved = [];
% for i =1:length(parameters_new)
%     pved = [pved;val{i}.*0+parameters_new{i}.scale(0.25*parameters{i}.boxLims(:,2))];
% end
% pvec = pved;
objh = @(p)evaluateMatchTwinModels(p, obj, setup_init,setup_init_new, parameters, statesRef,[],[]);
% run optimization
[v, p_opt, history] = unitBoxBFGS(pvec, objh, 'objChangeTol', 1e-7, 'gradTol', 1e-4, ...
...
[v2, p_opt2, history2] = unitBoxLM(pvec, objh2, 'maxIt', 25);
...
setup_opt1 = updateSetupFromScaledParameters(setup_init, parameters, p_opt);
setup_opt1_new = updateSetupFromScaledParameters(setup_init_new, parameters_new, p_opt2); 
...
setup_opt2_new = updateSetupFromScaledParameters(setup_init_new, parameters_new, p_opt2);
...
plotWellSols({wsRef, wsCoarse, wellSols_opt1, wellSols_opt1_new}, ...
