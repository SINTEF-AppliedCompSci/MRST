...
clc; clear;
load('EagleFord_HnP_Primary_rate_1e5_DOE.mat')
% load('Bakken_HnP_Primary_NFs_rate_3e4_DOE.mat')
...
for ts=1:numel(wsOld)
    qiProd = wsOld{ts}(2).components; %Inj then producer for HnP mat files
    if(isempty(qiProd))
...
for i = 1:numel(wsOldcorr)
    q_gs_corrected(i)=-3.051e+6.*wsOldcorr{i}(2).qGs;
end
...
%%
wsCombo = {wsOld,wsOldcorr};
names = {'wsOld','wsOldflashed'};
