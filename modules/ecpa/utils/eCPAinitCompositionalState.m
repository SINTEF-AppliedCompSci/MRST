function state = eCPAinitCompositionalState(model, p, T, s0, z0, DPDP, eos)
...
%}
    ...
    if hasModel
        ...
        assert(isa(model, 'ECPAThreePhaseCompositionalModel')|isa(model, 'ECPAThreePhaseCompositionalModelDP'));
        if nargin == 6
            eos = model.EOSModel;
        end
        G = model.G;
