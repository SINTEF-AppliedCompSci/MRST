...
for i = 1:ncomp
    ...
    if isfield(model.rock, 'Mechanisms') && isfield(model.rock.Mechanisms,'diffusion')
        ...
        [~, ii, jj] = permTensor(model.rock, G.griddim);
        % Compute T = C'*N / C'*C.
        T = zeros(size(cellNo));
        for k = 1 : size(ii, 2)
            T = T + (C(:, ii(k)) .* N(:, jj(k)));
        ...
        So_poro = s.faceUpstr(upco,sO.*model.rock.poro);
        
        Jg = s.Div(model.rock.Di(2,i)./model.rock.tau.*Sg_poro.*T.*s.Grad(yM{i}.*rhoG));
        Jo = s.Div(model.rock.Di(1,i)./model.rock.tau.*So_poro.*T.*s.Grad(xM{i}.*rhoO));
    ...
    nmole = mixture.getNumberOfMolecules();
    if i<=nmole
        eqs{ix} = (f_L{i}(twoPhase) - f_V{i}(twoPhase) + a)/barsa;
    else
        scale = value(f_V{i}(twoPhase))./value(f_L{i}(twoPhase));
        eqs{ix} = (f_L{i}(twoPhase).*scale - f_V{i}(twoPhase))/barsa;
    end
