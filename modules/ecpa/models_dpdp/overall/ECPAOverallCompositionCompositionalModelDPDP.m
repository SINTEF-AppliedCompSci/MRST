classdef ECPAOverallCompositionCompositionalModelDPDP < ECPAThreePhaseCompositionalModelDP
    % Overall composition model for dual-porosity dual-permeability compositional
    % problems using eCPA EOS.
    %
    % SYNOPSIS:
    %   model = ECPAOverallCompositionCompositionalModelDPDP(G, rock, fluid, compFluid)
    %
    % DESCRIPTION:
    ...
    %
    % SEE ALSO:
    %   `ThreePhaseCompositionalModelDP`, `ECPANaturalVariablesCompositionalModelDPDP`
    ...
    methods
        function model = ECPAOverallCompositionCompositionalModelDPDP(varargin)
            model = model@ECPAThreePhaseCompositionalModelDP(varargin{:});
        ...
        function [problem, state] = getEquations(model, state0, state, dt, drivingForces, varargin)
            [problem, state] = equationsCompositionalDPDP(state0, state, model, dt, ...
        ...
        function [state, report] = updateState(model, state, problem, dx, drivingForces)
            ...
            z = state.components;
            rm = 0;
            z_matrix = state.matrix.components;
            rm_matrix = 0;
            for i = 1:ncomp
                name = lower(cnames{i});
                ...
                % Wei: matrix
                cix_matrix = strcmpi(var0, [name,'_matrix']);
                if any(cix_matrix)
                    z0_matrix = z_matrix(:, i);

                    dz_matrix = dx{cix_matrix};
                    if isfinite(model.dzMaxAbs)
                        dz_matrix = sign(dz_matrix).*min(abs(dz_matrix), model.dzMaxAbs);
                    end
                    z_matrix(:, i) = min(max(z0_matrix + dz_matrix, 0), 1);

                    ok(i) = true;
                    [vars, ix_matrix] = model.stripVars(vars, {[name,'_matrix']});
                    removed(~removed) = removed(~removed) | ix_matrix;

                    rm_matrix = rm_matrix - (z_matrix(:, i) - z0_matrix);
                end
            end
            if any(ok)
                ...
                state.dz = dz;
                % Wei: matrix
                z_matrix(:, ~ok) = min(max(z_matrix(:, ~ok) + rm_matrix, 0), 1);
                z_matrix = bsxfun(@rdivide, z_matrix, sum(z_matrix, 2));
                state.matrix.components = z_matrix;
                dz_matrix = model.computeChange(state.matrix.components - state0.matrix.components, s_hc);
                state.matrix.dz = dz_matrix;
            else
                state.dz = zeros(1, ncomp);
                state.matrix.dz = zeros(1, ncomp);
            end
            ...
            if problem.iterationNo == 1
                ...
                % wei
                state.switched_matrix = false(model.G.cells.num, 1);
                state.switchCount_matrix = zeros(model.G.cells.num, 1);
            end
            twoPhase0 = state.L < 1 & state.L > 0;
            twoPhase0_matrix = state.matrix.L < 1 & state.matrix.L > 0;
            ...
            state.matrix.components = ensureMinimumFraction(state.matrix.components, minz);
            ...
            dx_matrix = model.computeChange(state0.matrix.x - state.matrix.x, s_hc);
            dy = model.computeChange(state0.y - state.y, s_hc);
            dy_matrix = model.computeChange(state0.matrix.y - state.matrix.y, s_hc);
            dxy = max(dx, dy);
            dxy_matrix = max(dx_matrix, dy_matrix);
            state.dz = max(state.dz, dxy);
            state.matrix.dz = max(state.matrix.dz, dxy_matrix);

            twoPhase = state.L < 1 & state.L > 0;
            twoPhase_matrix = state.matrix.L < 1 & state.matrix.L > 0;
            switched = twoPhase0 ~= twoPhase;
            switched_matrix = twoPhase0_matrix ~= twoPhase_matrix;
            ...
            state.switchCount_matrix = state.switchCount_matrix + double(switched_matrix);
            % Set minimum phase composition
            state.x = ensureMinimumFraction(state.x, minz);
            state.y = ensureMinimumFraction(state.y, minz);
            state.matrix.x = ensureMinimumFraction(state.matrix.x, minz);
            state.matrix.y = ensureMinimumFraction(state.matrix.y, minz);
        end
    end
end
