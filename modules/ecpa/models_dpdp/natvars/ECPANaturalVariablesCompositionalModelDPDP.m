classdef ECPANaturalVariablesCompositionalModelDPDP < ECPAThreePhaseCompositionalModelDP
    % Natural variables model for dual-porosity dual-permeability compositional
    % problems using eCPA EOS.
    %
    % SYNOPSIS:
    %   model = ECPANaturalVariablesCompositionalModelDPDP(G, rock, fluid, compFluid)
    %
    % DESCRIPTION:
    ...
    methods
        function model = ECPANaturalVariablesCompositionalModelDPDP(G, rock, fluid, mixture, varargin)
            model = model@ECPAThreePhaseCompositionalModelDP(G, rock, fluid, mixture, varargin{:});
            ...
            if isempty(model.inputdata)
                ...
                model.operators_matrix = setupOperatorsTPFA(model.G, model.rock_matrix); 
                ...
                model.operators_matrix = setupOperatorsTPFA(model.G, model.rock_matrix, opts_matrix{:}); 
            end
            model.operators.pv_matrix = operators_matrix.pv;
        end
        
        function [problem, state] = getEquations(model, state0, state, dt, drivingForces, varargin)
            [problem, state] = eCPAequationsNaturalVariablesDPDP(state0, state, model, dt, ...
        ...
        function [state, report] = updateState(model, state, problem, dx, drivingForces)
            ...
            [state, report] = updateState@ECPAThreePhaseCompositionalModelDP(model, state, problem, dx, drivingForces);
        ...
        function [convergence, values, names] = checkConvergence(model, problem, varargin)
            [convergence, values, names] = checkConvergence@ECPAThreePhaseCompositionalModelDP(model, problem, varargin{:});
        ...
        function [state, report] = updateAfterConvergence(model, state0, state, dt, drivingForces)
            [state, report] = updateAfterConvergence@ECPAThreePhaseCompositionalModelDP(model, state0, state, dt, drivingForces);
        ...
        function state = validateState(model, state)
            state = validateState@ECPAThreePhaseCompositionalModelDP(model, state);
