classdef ECPAThreePhaseCompositionalModelDP < ReservoirModelDP
    % Base class for dual-porosity compositional models using the ECPA equation
    %  of state
    %
    % SYNOPSIS:
    ...
    methods
        function model = ECPAThreePhaseCompositionalModelDP(G, rock, fluid, compFluid, varargin)
            opt = struct('inputdata', []);
            [opt, extra] = merge_options(opt, varargin{:});
            model = model@ReservoirModelDP(G, rock, fluid, 'inputdata', opt.inputdata);
        ...
        function [state, report] = updateState(model, state, problem, dx, drivingForces)
            p0 = state.pressure;
            [state, report] = updateState@ReservoirModelDP(model, state, problem, dx, drivingForces);
            range = max(p0) - min(p0);
        ...
        function state = computeFlash(model, state, dt, iteration)
            ...
            %Wei: matrix
            [state.matrix, report.matrix] = model.EOSNonLinearSolver.solveTimestep(state.matrix, dt, model.EOSModel);
            ...
            Z_V = state.Z_V;
            % Wei
            L_matrix = state.matrix.L;
            Z_L_matrix = state.matrix.Z_L;
            Z_V_matrix = state.matrix.Z_V;
            ...
            if isprop(propmodel, 'volumeShift') && isempty(propmodel.volumeShift)
                ...
                volV = (1-L).*Z_V;
                % Wei
                volL_matrix = L_matrix.*Z_L_matrix;
                volV_matrix = (1-L_matrix).*Z_V_matrix;
            else
                ...
                %Wei
                volL_matrix = L_matrix./propmodel.computeMolarDensity(model.EOSModel, state.matrix.pressure, state.matrix.x, Z_L_matrix, state.matrix.T, true);
                volV_matrix = (1-L_matrix)./propmodel.computeMolarDensity(model.EOSModel, state.matrix.pressure, state.matrix.y, Z_V_matrix, state.matrix.T, false);
            end
            volT = volL + volV;
            sL = volL./volT;
            % Wei
            volT_matrix = volL_matrix + volV_matrix;
            sL_matrix = volL_matrix./volT_matrix;
            ...
            if nph > 2
                ...
                for i = 1:numel(ph)
                    ...
                    void_matrix = void_matrix - model.getProp(state.matrix, ['s', ph(i)]);
                end
            end
            l = model.liquidPhase;
            v = model.vaporPhase;
            ...
            % Wei
            state.matrix = model.setProp(state.matrix, ['s', l], void_matrix.*sL_matrix);
            state.matrix = model.setProp(state.matrix, ['s', v], void_matrix.*(1-sL_matrix));

            assert(all(all(state.s >= 0)), 'Negative saturations after flash.');
            assert(all(all(state.matrix.s >= 0)), 'Negative saturations after flash.');
        end
    ...
    methods (Access=protected)
       function [v_comp, tol_comp, names_comp, isComponent] = getComponentConvergenceValues(model, problem)
            ...
            rho_matrix = state.matrix.rho;
            s_matrix = state.matrix.s;
            pv_matrix = model.operators.pv_matrix;
            mass_matrix = bsxfun(@times, pv_matrix, rho_matrix.*s_matrix);
            ncomp = numel(cnames);
            for i = 1:ncomp
                f = strcmpi(names, cnames{i});
                f_matrix = strcmpi(names, [cnames{i},'_matrix']);
                if any(f)
                    isComponent(f) = true;
                end
                if any(f_matrix)
                    isComponent(f_matrix) = true;
                end
            end
            ...
            if model.useIncTolComposition
                ...
                massT = sum(mass(:, phix), 2) + sum(mass_matrix(:, phix), 2);
                scale = dt./massT;
                if numel(snames)
       ...
       function [v_f, tol_f, names_f, isFugacity] = getFugacityConvergenceValues(model, problem)
            ...
            scale_matrix = sum(s(state.matrix.flag == 0, phix), 2);
            fuga = problem.equations(isFugacity);
            numFgfa = sum(isFugacity);

            v_f = cellfun(@(x) norm(value(x).*scale, inf), fuga(1:0.5*numFgfa));
            v_f_matrix = cellfun(@(x) norm(value(x).*scale_matrix, inf), fuga(1+0.5*numFgfa:end));
            v_f = [v_f,v_f_matrix];
