classdef ECPACompositionalViscosityLV < Viscosity
    properties
        useCompactEvaluation = true;
    end
    
    methods
        function gp = ECPACompositionalViscosityLV(model, varargin)
        ...
        function mu = evaluateOnDomain(prop, model, state)
            ...
            mu{L_ix} = pm.computeViscosity(eos, p, x, Z{L_ix}.Z, T, true);
            ...
            if prop.useCompactEvaluation && ~all(twoPhase)
                ...
                if any(twoPhase)
                    ...
                    muV(twoPhase) = pm.computeViscosity(eos, p(twoPhase), y, Z{V_ix}.Z(twoPhase), T(twoPhase), false);
                end
            else
                muV = pm.computeViscosity(eos, p, y, Z{V_ix}.Z, T, false);
            end
            mu{V_ix} = muV;
