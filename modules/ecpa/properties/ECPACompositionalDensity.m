classdef ECPACompositionalDensity < StateFunction
    ...
    methods
        function gp = ECPACompositionalDensity(model, varargin)
        ...
        function rho = evaluateOnDomain(prop, model, state)
            ...
            rhoL = pm.computeDensity(eos, p, x, Z{L_ix}.Z, T, true);
            if prop.useCompactEvaluation
                [~, ~, twoPhase] = model.getFlag(state);
                if all(twoPhase)
                    rhoV = pm.computeDensity(eos, p, y, Z{V_ix}.Z, T, false);
                else
                    rhoV = rhoL;
                    if any(twoPhase)
                        y2ph = cellfun(@(x) x(twoPhase), y, 'UniformOutput', false);
                        rhoV(twoPhase) = pm.computeDensity(eos, p(twoPhase), y2ph, Z{V_ix}.Z(twoPhase), T(twoPhase), false);
                    end
                end
            else
                rhoV = pm.computeDensity(eos, p, y, Z{V_ix}.Z, T, false);
