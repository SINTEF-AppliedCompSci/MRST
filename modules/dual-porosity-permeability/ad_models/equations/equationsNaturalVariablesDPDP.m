function [problem, state] = equationsNaturalVariablesDPDP(state0, state, model, dt, drivingForces, varargin)
% Equations for natural variables formulation for dual-porosity dual-permeability model
...
dpO_matrix    = s_matrix.Grad(p_matrix) - rhoOf_matrix.*gdz;
...
dpG_matrix= s_matrix.Grad(pG_matrix) - rhoGf_matrix.*gdz;
...
pv0 = pv;
% Wei
pv_matrix = model.operators_matrix.pv;
...
if model.water
    ...
    [vW, mobW, upcw, bW, rhoW, vW_matrix, mobW_matrix, upcw_matrix, bW_matrix, rhoW_matrix] = deal([]);
end
%% Wei: Transfer
...
compFlux_matrix = zeros(model.G.faces.num, ncomp);
...
for i = 1:ncomp
    ...
    vi_matrix = rOvO_matrix.*s_matrix.faceUpstr(upco_matrix, xM_matrix{i}) + rGvG_matrix.*s_matrix.faceUpstr(upcg_matrix, yM_matrix{i});
    fluxes{ncomp+i} = vi_matrix;
    compFlux_matrix(model.operators_matrix.internalConn,i) = value(vi_matrix);
...
state.matrix.componentFluxes = compFlux_matrix;
if model.water
    mf = [model.fluid.rhoWS.*value(rWvW), value(rOvO), value(rGvG)];
else
    mf = [value(rOvO), value(rGvG)];
    mf_matrix = [value(rOvO_matrix), value(rGvG_matrix)];
end
state.massFlux = zeros(model.G.faces.num, 2 + model.water);
state.massFlux(model.operators.internalConn, :) = mf;

state.matrix.massFlux = zeros(model.G.faces.num, 2 + model.water);
state.matrix.massFlux(model.operators_matrix.internalConn, :) = mf_matrix;
if model.water
    eqs{ncomp+1} = water;
    names{ncomp+1} = 'water';
    types{ncomp+1} = 'cell';
