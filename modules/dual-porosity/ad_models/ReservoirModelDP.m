...
methods
    % --------------------------------------------------------------------%
    function model = ReservoirModelDP(G, varargin)
        % We are being called in format
        % ReservoirModel(G, rock, fluid, ...)
        rock_all = varargin{1};
        fluid_all = varargin{2};
        model = model@ReservoirModel(G, rock_all{1}, fluid_all{1}, varargin(3:end));
    ...
    function [state, report] = updateState(model, state, problem, dx, drivingForces)
        ...
        if ~isempty(restVars)
            ...
            for i = 1:numel(restVars)
                if strcmp(restVars{i}, 'pressure_matrix')
                    state.matrix.pressure_matrix = state.matrix.pressure;
                    state.matrix = model.updateStateFromIncrement(state.matrix, dx, problem, restVars{i}, model.dpMaxRel, model.dpMaxAbs);
                    state.matrix.pressure = state.matrix.pressure_matrix;
                end
            end
        end
...
%         end
    ...
    function varargout = evaluateRelPerm(model, sat, varargin)
        ...
        % Wei:Ensure that both fracture and matrix relative permeabilities are evaluated
        if nargin == 4
        ...
        elseif nph == 1
            ...
            varargout{1} = f.(['kr', names])(sat{:}, varargin{:});
        end
    end
