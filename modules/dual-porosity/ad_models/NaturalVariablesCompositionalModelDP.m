classdef NaturalVariablesCompositionalModelDP < ThreePhaseCompositionalModelDP
    % Natural variables model for dual-porosity compositional problems
    %
    % SYNOPSIS:
    %   model = NaturalVariablesCompositionalModelDP(G, rock, fluid, compFluid)
    %
    % DESCRIPTION:
    ...
    %
    % SEE ALSO:
    %   `ThreePhaseCompositionalModelDP`, `OverallCompositionCompositionalModelDP`
    properties
    ...
    methods
        function model = NaturalVariablesCompositionalModelDP(G, rock, fluid, mixture, varargin)
            model = model@ThreePhaseCompositionalModelDP(G, rock, fluid, mixture, varargin{:});
            % Wei: Set up the matrix and fracture operators with potentially 
            % provided options
            if isempty(model.inputdata)
                model.operators = setupOperatorsTPFA(model.G, model.rock); 
                operators_matrix = setupOperatorsTPFA(model.G, model.rock_matrix); 
            else
                ...
                model.operators = setupOperatorsTPFA(model.G, model.rock, opts{:});   
                operators_matrix = setupOperatorsTPFA(model.G, model.rock_matrix, opts_matrix{:}); 
            end
            model.operators.pv_matrix = operators_matrix.pv;
        end
        
        function [problem, state] = getEquations(model, state0, state, dt, drivingForces, varargin)
            [problem, state] = equationsNaturalVariablesDP(state0, state, model, dt, ...
        ...
        function [state, report] = updateState(model, state, problem, dx, drivingForces)
            ...
            % Wei
            active = contains(vars0, vars);
            vars_active = vars0(~active);
            for i = 1:numel(vars_active)
                vars_matrix{i} = [vars_active{i},'_matrix'];
            end
            active = contains(vars, vars_matrix);
            vars(active) = vars_active;

            [ds_matrix, deltax_matrix, deltay_matrix, dL_matrix, dx, vars] = model.getHyperbolicUpdates(problem, dx, vars, state.matrix);
            ...
            w_xy = min(w_x, w_y);
            % Wei
            w_sat_matrix = min(dsMax./max(abs(ds_matrix), [], 2), 1);
            w_x_matrix = min(dxMax./max(abs(deltax_matrix), [], 2), 1);
            w_y_matrix = min(dyMax./max(abs(deltay_matrix), [], 2), 1);
            w_xy_matrix = min(w_x_matrix, w_y_matrix);

            % Minimum relax factor
            w = min(w_sat, w_xy);
            w_matrix = min(w_sat_matrix, w_xy_matrix);
            % Update sat
            ...
            % Wei
            if any(ds_matrix(:) ~= 0)
                ds_relax_matrix = ds_matrix;
                ds_relax_matrix(:, ~isEoSPhase) = ds_matrix(:, ~isEoSPhase).*min(dsMax./max(abs(ds_matrix(:, ~isEoSPhase)), [], 2), 1);
                ds_relax_matrix(:, isEosPhase) = bsxfun(@times, ds_matrix(:, isEosPhase), w_matrix);
                s_uncap_matrix = state.matrix.s + ds_relax_matrix;
                if model.allowLargeSaturations
                    state.matrix.s = max(s_uncap_matrix, 0);
                else
                    state.matrix.s = capunit(s_uncap_matrix);
                    state.matrix.s = bsxfun(@rdivide, state.matrix.s, sum(state.matrix.s, 2));
                end
            else
                s_uncap_matrix = state.matrix.s;
            end
            % Update dx, dy
            ...
            % Wei
            if any(deltax_matrix(:) ~= 0) || any(deltay_matrix(:) ~= 0)
                state.matrix.x = max(state.matrix.x + bsxfun(@times, deltax_matrix, w_matrix), 0);
                state.matrix.y = max(state.matrix.y + bsxfun(@times, deltay_matrix, w_matrix), 0);

                state.matrix.x = capunit(state.matrix.x);
                state.matrix.x = bsxfun(@rdivide, state.matrix.x, sum(state.matrix.x, 2));

                state.matrix.y = capunit(state.matrix.y);
                state.matrix.y = bsxfun(@rdivide, state.matrix.y, sum(state.matrix.y, 2));
                % Update K-values
                isTwoPhase_matrix = model.getTwoPhaseFlag(state.matrix);
                state.matrix.x = ensureMinimumFraction(state.matrix.x, model.EOSModel.minimumComposition);
                state.matrix.y = ensureMinimumFraction(state.matrix.y, model.EOSModel.minimumComposition);

                state.matrix.K(isTwoPhase_matrix, :) = state.matrix.y(isTwoPhase_matrix, :)./state.matrix.x(isTwoPhase_matrix, :);
                xyUpdated_matrix = true;
            else
                xyUpdated_matrix = false;
            end
            ...
            % Wei
            if ~isempty(dL_matrix)
                state.matrix.L = capunit(state.matrix.L + w_matrix.*dL_matrix);
            end

            problem.primaryVariables = vars;
            [state, report] = updateState@ThreePhaseCompositionalModelDP(model, state, problem, dx, drivingForces);
            state = model.flashPhases(state, state0, s_uncap, xyUpdated, problem.iterationNo);
            state.matrix = model.flashPhases(state.matrix, state0.matrix, s_uncap_matrix, xyUpdated_matrix, problem.iterationNo);
            sT = sum(state.s, 2);
            ...
            % Wei
            sT_matrix = sum(state.matrix.s, 2);
            sT_hc_matrix = sum(state.matrix.s(:, isEoSPhase), 2);
            s_hc_matrix = sT_hc_matrix./sT_matrix;
            dx_matrix = model.computeChange(deltax_matrix, s_hc_matrix);
            dy_matrix = model.computeChange(deltay_matrix, s_hc_matrix);
            dz_matrix = model.computeChange(state.matrix.components - state0.matrix.components, s_hc_matrix);
            state.matrix.dz = max(dz_matrix, max(dx_matrix, dy_matrix));
        end
        ...
        function [convergence, values, names] = checkConvergence(model, problem, varargin)
            [convergence, values, names] = checkConvergence@ThreePhaseCompositionalModelDP(model, problem, varargin{:});
        ...
        function [state, report] = updateAfterConvergence(model, state0, state, dt, drivingForces)
            [state, report] = updateAfterConvergence@ThreePhaseCompositionalModelDP(model, state0, state, dt, drivingForces);
        ...
        function state = validateState(model, state)
            state = validateState@ThreePhaseCompositionalModelDP(model, state);
