function [problem, state] = equationsCompositionalDP(state0, state, model, dt, drivingForces, varargin)
...
for i = 1:ncomp
    ...
    if isfield(model.rock, 'Mechanisms') && isfield(model.rock.Mechanisms,'sorption')
        eqs{i+woffset+ncomp} = (1/dt).*( ...
            pv_matrix.*rhoO_matrix.*pvMult_matrix.*sO_matrix.*xM_matrix{i} - pv_matrix.*rhoO0_matrix.*pvMult0_matrix.*sO0_matrix.*xM0_matrix{i} + ...
            pv_matrix.*rhoG_matrix.*pvMult_matrix.*sG_matrix.*yM_matrix{i} - pv_matrix.*rhoG0_matrix.*pvMult0_matrix.*sG0_matrix.*yM0_matrix{i} +...
        ...
        eqs{i+woffset+ncomp} = (pv_matrix/dt).*( ...
    ...
    fluxes{i+woffset+ncomp} = 0.*fluxes{i+woffset};
end
