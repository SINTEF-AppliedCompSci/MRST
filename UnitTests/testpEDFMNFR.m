...
%   Detailed explanation goes here
    ...
    try
        %% This code requires ADFNE and the Statistics & Machine Learning Toolbox
        set1 = Field(DFN('dim',3,'n',15,'dir',45,'ddir',-100,'minl',5,'mu',20, ...
            'maxl',20,'bbx',[tol,tol,tol,physdim(1)-tol, ...
            physdim(2)-tol,physdim(3)-tol],'dip',45,'ddip',-100, ...
            'shape','l','q',4),'Poly');
        set2 = Field(DFN('dim',3,'n',15,'dir',-45,'ddir',-100,'minl',5,'mu',20, ...
            'maxl',20,'bbx',[tol,tol,tol,physdim(1)-tol, ...
            physdim(2)-tol,physdim(3)-tol],'dip',45,'ddip',-100, ...
            'shape','l','q',4),'Poly');
    catch
    ...
    %% pEDFM PreProcessing
    % The preprocessing takes a while. The user can  remove line 147 to 159 and
    % instead load the preprocessed grid at line 162
    ...
    load('wspEDFMNFR.mat')
    load('statespEDFMNFR.mat')

    wsStructRef = vertcat(wspEDFMNFR{:});
    ...
    totalInfNorm = norm(qOs-qOsRef, inf) + norm(qGs-qGsRef, inf) + ...
