...
if(false)
    ...
    L=100;L_p=L/2;H=10
    G=cartGrid([10 2 1],[L L H]);
    ff=@(x) sin(pi*x/L_p)*0.5+x*0.08;
    ...
    Gt.cells.H=ones(Gt.cells.num,1)*10;
...
rock.perm=ones(G.cells.num,1).*G.cells.H*darcy*1000;
...
for i=1:numel(sat_vec);      
  sat=sat_vec(i);
      ...
      opt=struct( 'verbose',         false,...
         ...
         'solve_pressure', true,...
         'dhfz',dhfzp);
      [state,report] = simulateToSteadyStatePeriodic(state,Gp,G,bcp,psolver_transport,Transp,rock,fluid,[],opt);
      %%
      figure(1),clf
...
creport=struct('sim_ok',sim_ok,'newton_steps',newton_steps,'steps',steps);
