...
%---------------------------------Case 2----------------------------------%    
    ...
    case 2
        ...
        for i = 1:numel(cg_frac)
            fcg_fcells = frac_cells(p(frac_cells)==cg_frac(i)).';
            faces = [];
            for j = 1:numel(fcg_fcells)
                faces = [faces;G.cells.faces(G.cells.facePos(fcg_fcells(j)):G.cells.facePos(fcg_fcells(j)+1)-1,1)]; %#ok
            end
            ...
            K = convhull(xcent,ycent);
            in = inpolygon(G.cells.centroids(:,1),G.cells.centroids(:,2),xcent(K),ycent(K));
            CG.cells.interaction{cg_frac(i),1} = unique([CG.cells.interaction{cg_frac(i),1};find(in)]);
        %-----------------------See what's happening----------------------%
%             figure; plotGrid(G,'EdgeAlpha',0.05,'FaceColor','none'); axis tight off
%             outlineCoarseGrid(G.Matrix,p(1:G.Matrix.cells.num),'k')
%             xcent = G.faces.centroids(faces,1); ycent = G.faces.centroids(faces,2);
%             K1 = convhull(xcent,ycent);
%             in1 = inpolygon(G.cells.centroids(:,1),G.cells.centroids(:,2),xcent(K1),ycent(K1));
%             plotGrid(G,find(in),'EdgeAlpha',0,'FaceColor','b');
%             plotGrid(G,find(in1),'EdgeAlpha',0,'FaceColor','g');
