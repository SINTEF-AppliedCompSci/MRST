classdef ADI
   ...
   methods
      ...
      function u = rldecode(u, varargin)
          u.val = rldecode(u.val, varargin{:});
          u.jac = ADI.rldecodeJac(u.jac, varargin{:});
      end
   ...
   methods (Access=protected, Static)
        ...
        function J = rldecodeJac(J1, varargin)
        J   = cell(1, numel(J1));
        if (varargin{end}(end)==1)
            for k = 1:numel(J)
                Jtmp = J1{k};
                [r,c] = size(Jtmp);
                Jtmp = rldecode(Jtmp(:)', varargin{:});
                Jtmp = reshape(Jtmp, c, [])';
                J{k} = Jtmp;
            end
        else
