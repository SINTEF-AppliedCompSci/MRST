...
function [gradient1, result1, report1,gradient2,result2, report2] = solveAdjointTwinModels(model_1, model_2,solver, getState_1,getState_2,...
                                getObj, schedule_1,schedule_2, gradient1,gradient2, stepNo, varargin)
        % Solve a single linear adjoint step to obtain the gradient
        %
        % SYNOPSIS:
        ...
        validforces_1 = model_1.getValidDrivingForces();
        dt_steps_1 = schedule_1.step.val;

        validforces_2 = model_2.getValidDrivingForces();
        dt_steps_2 = schedule_2.step.val;

        current_1 = getState_1(stepNo);
        before_1  = getState_1(stepNo - 1);
        dt_1 = dt_steps_1(stepNo);

        current_2 = getState_2(stepNo);
        before_2  = getState_2(stepNo - 1);
         dt_2 = dt_steps_2(stepNo);
%         schedule_1.control.W(1).val = schedule_1.control.W(1).val-(current_2.wellSol(1).qWs+current_2.wellSol(1).qOs);
%         schedule_1.control.W(2).val = schedule_1.control.W(2).val-(current_2.wellSol(2).qWs+current_2.wellSol(2).qOs);
%         schedule_1.control.W(3).val = schedule_1.control.W(3).val-(current_2.wellSol(3).bhp);
%         schedule_1.control.W(4).val = schedule_1.control.W(4).val-(current_2.wellSol(4).bhp);
% 
% for i = 1:15
%     stat= getState_1(i);
%         schedule_2.control(i).W(1).val = schedule_2.control(i).W(1).val-(stat.wellSol(1).bhp);
%         schedule_2.control(i).W(2).val = schedule_2.control(i).W(2).val-(stat.wellSol(2).bhp);
%         schedule_2.control(i).W(3).val = schedule_2.control(i).W(3).val-(stat.wellSol(3).qWs+stat.wellSol(3).qOs);
%         schedule_2.control(i).W(4).val = schedule_2.control(i).W(4).val-(stat.wellSol(4).qWs+stat.wellSol(4).qWs);
% end
        lookupCtrl_1 = @(step) schedule_1.control(schedule_1.step.control(step));
        lookupCtrl_2 = @(step) schedule_2.control(schedule_2.step.control(step));

        % get forces and merge with valid forces
        forces_1 = model_1.getDrivingForces(lookupCtrl_1(stepNo));
        forces_1 = merge_options(validforces_1, forces_1{:});
        if isa(model_1, 'ReservoirModel') && ~isempty(model_1.FacilityModel)
            model_1.FacilityModel = model_1.FacilityModel.validateModel(forces_1);
        else
            model_1 = model_1.validateModel(forces_1);
        end

        % Initial state typically lacks wellSol-field, so add if needed
        if stepNo == 1
            before_1 = model_1.validateState(before_1);
        end
 
         % get forces and merge with valid forces
        forces_2 = model_2.getDrivingForces(lookupCtrl_2(stepNo));
        forces_2 = merge_options(validforces_2, forces_2{:});
        if isa(model_2, 'ReservoirModel') && ~isempty(model_2.FacilityModel)
            model_2.FacilityModel = model_2.FacilityModel.validateModel(forces_2);
        else
            model_2 = model_2.validateModel(forces_2);
        end

        % Initial state typically lacks wellSol-field, so add if needed
        if stepNo == 1
            before_2 = model_2.validateState(before_2);
        end
        ...
        % This slightly messy setup is made to support hysteresis models,
        % where the forward and backwards equations are very different.
        problem_1 = model_1.getAdjointEquations(before_1, current_1, dt_1, forces_1, ...
                                    'reverseMode', false, 'iteration', inf);
          
        problem_2 = model_2.getAdjointEquations(before_2, current_2, dt_2, forces_2, ...
                                    'reverseMode', false, 'iteration', inf);

        if stepNo < numel(dt_steps_1)
            after_1    = getState_1(stepNo + 1);
            dt_next_1 = dt_steps_1(stepNo + 1);
            [current_1, primaryVars_1] = model_1.getReverseStateAD(current_1);
            assert(isequal(current_1.primaryVariables, primaryVars_1))
            % check if controls for for stepNo/stepNo+1 are different
            if diff(schedule_1.step.control([stepNo;stepNo+1])) == 0
                forces_p_1 = forces_1;
            else
                % get forces and merge with valid forces
                forces_p_1 = model_1.getDrivingForces(lookupCtrl_1(stepNo + 1));
                forces_p_1 = merge_options(validforces_1, forces_p_1{:});
                if isa(model_1, 'ReservoirModel') && ~isempty(model_1.FacilityModel)
                    model_1.FacilityModel = model_1.FacilityModel.validateModel(forces_p_1);
                else
                    model_1 = model_1.validateModel(forces_p_1);
                end
            end
            problem_p_1 = model_1.getAdjointEquations(current_1, after_1, dt_next_1, forces_p_1,...
                                'iteration', inf, 'reverseMode', true);                
        else
            problem_p_1 = [];
        end
        ...
        if stepNo < numel(dt_steps_2)
            after_2    = getState_2(stepNo + 1);
            dt_next_2 = dt_steps_2(stepNo + 1);
            [current_2, primaryVars_2] = model_2.getReverseStateAD(current_2);
            assert(isequal(current_2.primaryVariables, primaryVars_2))
            % check if controls for for stepNo/stepNo+1 are different
            if diff(schedule_2.step.control([stepNo;stepNo+1])) == 0
                forces_p_2 = forces_2;
            else
                % get forces and merge with valid forces
                forces_p_2 = model_2.getDrivingForces(lookupCtrl_2(stepNo + 1));
                forces_p_2 = merge_options(validforces_2, forces_p_2{:});
                if isa(model_2, 'ReservoirModel') && ~isempty(model_2.FacilityModel)
                    model_2.FacilityModel = model_2.FacilityModel.validateModel(forces_p_2);
                else
                    model_2 = model_2.validateModel(forces_p_2);
                end
            end
            problem_p_2 = model_2.getAdjointEquations(current_2, after_2, dt_next_2, forces_p_2,...
                                'iteration', inf, 'reverseMode', true);                
        else
            problem_p_2 = [];
        end
               ...
               problem2 = problem_2;
               for i=1:5 
                   problem_2.equations{i} = value(problem_2.equations{i});
                   problem_2.state = value(problem_2.state); 

                   problem_1.equations{i} = value(problem_1.equations{i});
                   problem_1.state = value(problem_1.state); 
                  
               end
                problem_2.state.primaryVariables = problem2.state.primaryVariables; 
                problem_2.state.FacilityState.names = problem2.state.FacilityState.names;

                problem_1.state.primaryVariables = problem1.state.primaryVariables; 
                problem_1.state.FacilityState.names = problem1.state.FacilityState.names; 
        [gradient1, result1, rep1] = solver.solveAdjointProblem(problem_p_1,...
                                    problem1, gradient1, getObj(stepNo,model_1,model_2,problem1.state,problem_2.state), model_1, ...
                                    varargin{:});
        [gradient2, result2, rep2] = solver.solveAdjointProblem(problem_p_2,...
                                    problem2, gradient2, getObj(stepNo,model_1,model_2,problem_1.state,problem2.state), model_2, ...
                                    varargin{:});
        report1 = struct();
        report1.Types = problem_1.types;
        report1.LinearSolverReport = rep1;
                
        report2 = struct();
        report2.Types = problem_2.types;
        report2.LinearSolverReport = rep2;
    end
