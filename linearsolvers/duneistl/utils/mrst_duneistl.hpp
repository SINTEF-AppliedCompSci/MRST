...
namespace mrst{
  ...
  makeAmgPreconditioner(Dune::MatrixAdapter<MatrixType, VectorType, VectorType> & linearoperator,
			boost::property_tree::ptree& lprm){
      typedef Dune::MatrixAdapter<MatrixType, VectorType, VectorType> OperatorType;
      std::shared_ptr< Dune::Preconditioner<VectorType,VectorType> > preconditioner;
      boost::property_tree::ptree prm = lprm.get_child("amg");
  ...
  public:
    ...
    void makeSolver(double tol, int maxiter,MatrixType& matrix){
      matrix_ = matrix;
      makeSolver(double tol, int maxiter);
    }
	...
	//preconditioner_ = makeAmgPreconditioner<Smoother, MatrixType, VectorType>(*linearoperator_,prm_);
	preconditioner_ = makeAmgPreconditioners<MatrixType, VectorType>(*linearoperator_,prm_);
      }else if( prm_.get<std::string>("preconditioner") == "cpr"){
       	//preconditioner_ = makeCpr<MatrixType, VectorType>(*linearoperator_,prm_);
	int pressureVarIndex = 0;
	boost::property_tree::ptree prm_cpr = prm_get_child("cpr");
	...
	preconditioner_.reset(new TwoLevelMethod(linearoperator_,
						 finesmoother_,
