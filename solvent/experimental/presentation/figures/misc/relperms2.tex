...
\begin{tikzpicture}[scale=1]
  ...
  \begin{axis}[ 
    ...
    \def\kri(#1){(1-\sgrm)^2*(max(min(((#1) - \sorm)/(1 - \sorm - \sgrm),1),0))^2}

    \def\krm(#1){(1-\sgrm)^2*(max(min(((#1) - \sorm)/(1 - \sorm - \sgrm),1),0))}

    \def\kr(#1){0.5*\kri(#1) + 0.5*\krm(#1)}
    ...
    \addplot [black, domain = \sorm:(1-\sgrm), samples = 500, thick] {\kri(x)};
    \addplot [black, domain = \sorm:(1-\sgrm), samples = 500, thick] {\krm(x)};
    \addplot [black, domain = \sorm:(1-\sgrm), samples = 500, thick] {\kr(x)};
