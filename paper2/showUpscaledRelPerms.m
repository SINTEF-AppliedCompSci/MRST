...
for i=1:numel(surf_topos) 
   fluid{i} = makeFluidModel(avgAquif, 'residual', false, ...
...
p0  = Gt.cells.z(:)*norm(gravity)*fluid{1}.rhoW;
markers={'k','g','m','r','b'};
for k=1:numel(fluid)
   for j=1:numel(sGv)
      sGa(1:end) = sGv(j);
...
% The numerically upscaled models are loaded from a precomputed file.
us = load('data/upscaled_relperm_theta');
