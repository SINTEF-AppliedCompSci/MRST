...
%[wellSols, states] = runMrstADI(x0, Gt, systemOG, control,'force_step',force_timesteps);
t2=tic;
[wellSols, states] = runMrstADI(x0, Gt, systemOG, control,'force_step',force_timesteps,'dt_min', 0.5*year,'report_all',false);
t2=toc(t2);
...
%%
%state=states{5}
%diff=(state.rs-minRs(state.pressure,state.s(:,1),state.sGmax,fluid, Gt)./state.s(:,1))/(fluid.dis_max)*50;
%plot((diff)