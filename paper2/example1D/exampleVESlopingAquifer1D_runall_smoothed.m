   ...
   for res_fluid = [false, true]
      for depth = [2300, 1300]
         ...
         for i = 1:numel(ff_names); 
            ...
            switch fluid_case
              ...
              case 'sharp interface'    
                fluid = addVERelperm(fluid, Gt,...
                                     ...
                                     'surf_topo' , surf_topo); 
              case 'linear cap.'
                fluid = addVERelpermCapLinear(fluid,0.2 * max(opt.Gt.cells.H) *             ...
            ...
            dt          = [dt; T_inj - sum(dt)]; 
            dt_post     = linspace(0.5 * year, dt_mig, 5)'; 
            dt_shift    = sum(dt_post); 
            dt_post     = [dt_post; ones(floor((T_mig - dt_shift) / dt_mig), 1) * dt_mig]; 
         ...
         if ~exist('test_data_smoothed', 'dir')
            mkdir('test_data_smoothed'); 
         end
         save(['test_data_smoothed/', fname], 'results', 'z', 'xc', 'opt')   
