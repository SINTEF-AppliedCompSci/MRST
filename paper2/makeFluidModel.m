...
if (opt.residual)
    [res_gas,res_oil] = deal(.21, .11);
else
    [res_gas,res_oil] = deal(0);
end
...
switch opt.co2_type
    ...
    case 'coolprops_table'
        ...
        co2=pvt2Table({co2},linspace(100,500,40)*barsa,linspace(min(T_res)-10,max(T_res)+10,40));co2=co2{1};
        f.bG =@(p) co2.density(p,T_res)./f.rhoGS;
    ...
    switch opt.fluidType
        case 'simple'
            ...
            f.pcOG    = @(sg, p, varargin) ...
                g*( f.rhoOS.*f.bO(p)-f.rhoGS.*f.bG(p) ).*(sg).*aquifer.Gt.cells.H;
            f.res_gas = 0;
            f.res_oil = 0;
        ...
        case 'linear cap'
            f = addVERelpermCapLinear(f, ...
                ...
                'H',           aquifer.Gt.cells.H,'kr_pressure',true);
            
        case 'S table'
            table_co2_1d = makeVEtables(...
                'invPc3D',     @(p) max((C./(p+C)).^(1/alpha),res_oil), ...
                'is_kscaled',  false, ...
                'kr3D',        @(s) s.^beta, ...
                'drho',        drho, ...
                'Gt',          aquifer.Gt, ...
