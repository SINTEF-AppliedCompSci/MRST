%mydir='/home/hnil/heim/SVN/simmatlab/branches/mrst-reorg/modules/co2lab-matlab_paper/papers'
mydir='';
param=[0.2,0.1];
for entrypress=[false,true]
    ...
    ff_names={ff_names{end}}
    fluid_names={fluid_names{end}};
    leg={};
    %fluid={};
    for i=1:numel(fluid_names)
        fluid{end+1} =  makeVEFluidsForTest(fluidADI, fluid_names{i},...
            'res_gas', 0,...
            'res_water', 0,...
            'Gt', Gt, 'rock', rock,'res_water',srw,'res_gas',src);
    ...
    %{
        fluid=addVERelperm1DTables(fluidADI,...
                                   'res_water',srw,...
                                   'res_gas',src,...
                                   'height',Gt.cells.H,...
    ...
    %{
        ...
        for i=1:msize(1)
            for j=1:msize(2)
                [s,pc,kr,SH,krH, s_max,fval]= veRelpermTester(H(i,j), p_ref, fluid_org, Hr, 'samples',1000,'hs_max',H_max_in(i,j),'kscale',kscale,'const_res',false);
                org_SH(i,j)=SH;
                org_krH(i,j)=krH;
                org_s_max(i,j)=s_max;
                [s,pc,kr,SH,krH, s_max,fval]= veRelpermTester(H(i,j), p_ref, fluid_org2, Hr, 'samples',1000,'hs_max',H_max_in(i,j),'kscale',kscale,'const_res',true);
                sara_SH(i,j)=SH;
                sara_krH(i,j)=krH;
        ...
        scatteredInterpolant =@(x,y,z,method,extrap) TriScatteredInterp(x,y,z,method);%..., METHOD
        %%
        ...
        print([mydir,'figs/relperm_hyst',myname,'.eps'],'-depsc2')
        ...
        print([mydir,'figs/pc_hyst',myname,'.eps'],'-depsc2')
        %%
          ...
          %%
           ...
           if(entrypress)
            myname=['entry_',myname];
           else
            myname=['noentry_',myname];   
           end
           [s,pc,kr,SH_org,krH, s_max_org,fval_org]= veRelpermTester(H_org, p_ref, fluid_org, H_res, 'samples',1000,'hs_max',H_org_max,'kscale',kscale,'const_res',false)          
           [s,pc,kr,SH_mod,krH, s_max_mod,fval_mod]= veRelpermTester(H_mod, p_ref, fluid_org2, H_res, 'samples',1000,'hs_max',H_mod_max,'kscale',kscale,'const_res',true) 
           if(kk==1)
           ...
           ylabel('depth')
           xlabel('s_{n}')
           %[figx figy] = dsxy2figxy_new([s_org(5),1-5/25],[fluid_org.res_gas*s_org(5),1-5/25])
           ah=H_org/2;
           ...
           set(gca,'Ydir','reverse')
           %%
           print([mydir,'figs/orginal_reconstuct',myname,'.eps'],'-depsc2')
           %
           ...
           print([mydir,'figs/mod_reconstuct',myname,'.eps'],'-depsc2')
           end
           %%
