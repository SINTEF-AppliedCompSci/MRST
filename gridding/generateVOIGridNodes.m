function [players, t, bdyID] = generateVOIGridNodes(GC, packed, WR, layerRf, opt)
% Generate 3D points of all VOI grid surfaces and connectivity list 
% corresponding to the 2D points
%
% SYNOPSIS:
%   [players, t, bdyID] = generateVOIGridNodes(GC, packed, WR, layerRf, opt)
%
% PARAMETERS:
%  GC           - CPG grid struct
%  packed       - Struct of VOI information, generated by `allInfoOfVolume`
%  WR           - Struct of 2D WR points, generated by 
%                 `prepareWellRegionNodes2D`
%  layerRf      - Number of refined layers in each VOI layer
%
% RETURNS:
%  players  - Points of all VOI grid surfaces
%  t        - Conectivity list
%  bdyID    - ID of outer-boundary in points p

    % Reference layer
    layer = ceil( length(packed.nodes)/2 );
    switch opt.gridType
        case 'triangular'
    ...
    [p, t, bdyID] = generator(GC, packed, WR, layer, opt);
    
    % Get layer points
    players = getLayerPoints(GC, packed, layerRf, layer, p, bdyID);
end

function [p, t, bdyID] = triangularPts(G, packed, WR, layer, opt)
    % Assign VOI data
    bnV = packed.bdyNodes{layer};
    ...
    % Get boundary points
    pib = pW(bnW,:); % Internal boundary points
    pob = G.nodes.coords(bnV, [1,2]); % Outer boundary nodes points
...
function [p, t, bdyID] = VoionoiPts(G, packed, WR, layer, opt)
    ...
    % Get internal boundary points and auxiliary point of WR
    pW   = [vertcat(WR.points.cart); vertcat(WR.points.rad)];
    tW   = WR.connlist;
    bnW  = WR.bdnodes;
    pib  = pW(bnW,:);
...
% -------------------------------------------------------------------------
function players = getLayerPoints(GC, packed, layerRf, layer, p, bdyID)
    ...
    for k = 1 : length(nV)
        ...
        xcRef = mean( GC.nodes.coords(bnV{layer}, 1) );
        ycRef = mean( GC.nodes.coords(bnV{layer}, 2) );
