function [players, playersXY, wellbores] = generateHWGridNodes(GV, packed, well, radPara)
% Generate 3D point, and 2D points for the radial HW grid
%
% PARAMETERS:
%  GV           - Unstructured VOI grid struct
%  packed       - Struct of HW information, generated by `allInfoOfRegion`
%  well         - Struct of the well infomation
%  radPara      - Parameters for generating the radial grid, see 
%                 `ReConstructToRadialGrid`
%
% RETURNS:
%  players    - Points of all HW grid surfaces
%  playersXY  - players in xy plane, used to compute the radial
%               transmissibility factors
%  wellbores  - Struct of the points of casing and tubing, used to generate
%               nodes and segments for multi-segment well
...
% Generate pure circular radial points for single well nodes ii
    ...
    pob = bsxfun(@minus, pob0, origin); % Points in wCenter coordinate
    pob = pob(:,1:2);
    
    % Theta and radius of the boundary nodes
    [theta, rob] = cart2pol(pob(:,1), pob(:,2));
    assert(all(rob > rM), ['Max radius of the radial grid is greater than '...
        'the boundary radius, please reduce the value']);
    ...
    if isfield(well, 'screenRadius')
        ...
        assert(rS < rW, ...
            'The tubing radius must be less than the casing radius!')
...
% Generate gradual radial points for single well nodes th
