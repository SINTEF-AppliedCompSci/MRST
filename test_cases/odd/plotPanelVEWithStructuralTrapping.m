function plotPanelVEWithStructuralTrapping(G, Gt, W, sol, t, vol, varargin)
...
%   t       - Time
%   vol     - Vector of (free, res_trapped, struct_trapped) volume
%   opt     - Various options
...
pie(max(vol, eps),  ... 
   { ['free '       num2str(round(vol(1))/total_volume * 100) ' %'], ...
     ['trapped(r) ' num2str(round(vol(2))/total_volume * 100) ' %'], ...
     ['trapped(s) ' num2str(round(vol(3))/total_volume * 100) ' %'] });

% $$$ str1 = ['free '       num2str(round(vol(1))/total_volume * 100) ' %'];
% $$$ str2 = ['trapped(r) ' num2str(round(vol(2))/total_volume * 100) ' %'];
% $$$ str3 = ['trapped(s) ' num2str(round(vol(3))/total_volume * 100) ' %'];
% $$$ pie(max(vol, eps), {str1, str2, str3});

title(['Total volume: ', num2str(round(convertTo(total_volume, mega*meter^3))), ...
       ' M m^3']);
