...
for i = 1:numel(fracScaled)
    fracScaled(i).points = [fracScaled(i).points(:,1)./G.Domain(1), ...
                            fracScaled(i).points(:,2)./G.Domain(2), ...
                            fracScaled(i).points(:,3)./G.Domain(3)];
    Ar(i) = polyArea3D(fracplanes(i).points);
    Ar_scaled(i) = polyArea3D(fracScaled(i).points);
end
lx = G.Domain(1); ly = G.Domain(2); lz = G.Domain(3);
...
%
[fraCells,remove] = markcells(G, fracplanes, 'Sign', Sign);
if ~isempty(remove)
...
for i = 1:numel(fracplanes)
    ...
    tripoints = uniquetol([cc;nc;gridpoints;tripoints], utol, 'ByRows', true);
    %
    tri = delaunayTriangulation(tripoints);
    
    fieldname = ['Plane',num2str(i)];
    Gf.(fieldname) = gridFractureDistmesh(G, fracplanes(i), ...
                     fracScaled(i), esize_scaled, 'type', 'pebi');
...
A = fracplane.normal(1); B = fracplane.normal(2); C = fracplane.normal(3);
D = -dot(normal,points(1,:));

dist = abs((A*cent(triToUse,1) + B*cent(triToUse,2) + C*cent(triToUse,3) + D)./norm(fracplane.normal));
dV = dist.*V(triToUse);
