...
% make grid sloping
G.nodes.coords(:,3) = 2000 + G.nodes.coords(:,3) - G.nodes.coords(:,1) * 0.05;
G = computeGeometry(G);
...
bc = pside([], Gt, 'xmin', Gt.cells.z(1) * rhow * g(3));
bc = pside(bc, Gt, 'xmax', Gt.cells.z(end) * rhow *g(3));
bc.sat = repmat([1 0], 2, 1);

% well
wcell = ceil(xres/5);
inj_rate = 0.075 * mega * 1e3 / year/ co2_rho;
...
for i = 1:numel(states)
    ...
    clf; plotCellData(G, s, 'edgealpha', 0.1); view(0,0);
    pause(0.1)
end
