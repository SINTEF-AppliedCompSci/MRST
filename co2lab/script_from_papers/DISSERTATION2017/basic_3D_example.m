...
schedule.step.val = [repmat(year/12, 24, 1); ... % 24 months
                     repmat(year/4, 8, 1)];      % two years
    
 %repmat(1* year, 10, 1)];    % 10 years
...
G_co2 = extractSubgrid(G, find(sat_end > 0.05)); % cells with CO2 saturation
                                                 % more than 0.05
clf; plotGrid(G, 'facecolor', 'none');  % plot outline of simulation grid
plotGrid(G_co2, 'facecolor', 'red'); % plot cells with CO2 in red
view(35, 35);

%% Plot saturation at a cross-section of the grid
[I, J, K] = ndgrid(1:G.cartDims(1), 1:ceil(G.cartDims(2)/2), 1:G.cartDims(3));
h_ixs = sub2ind(G.cartDims, I(:), J(:), K(:));
Ghalf = extractSubgrid(G, h_ixs);
clf;
subplot(1,2,1);
plotCellData(Ghalf, sat_end(h_ixs));
view(148, 54);
subplot(1,2,2);
plotCellData(Ghalf, states{end}.pressure(h_ixs));
view(148, 54);
set(gcf, 'position', [200, 200, 1500, 650]);