...
\begin{tikzpicture}[node distance = 2cm, auto]
  ...
  \begin{scope}[shift = {(2.5,0)}]
    \def\y{4/3-1}
    \def\x{-4/3+1}
    \draw[fill = black!65!ye] (-1,\y) to[out=0, in=130] (0,\y) to[out=-50, in=180](1,\y) -- (1,1) -- (-1,1) -- (-1,\y);
    \draw[fill = ye] (-1,\y) to[out=0, in=130] (0,\y) to[out=-50, in=180](1,\y) --
                              ( 1,\x) to[out=180, in=-50](0,\x) to[out=130, in=0](-1,\x) -- (-1,\y);
    \draw[fill = re!65!ye] (1,\x) to[out=180, in=-50](0,\x) to[out=130, in=0](-1,\x) -- (-1,-1) -- (1,-1) -- (1,\x);
    
    \node[color = white] at (0,\y + 0.3) {\tiny oil + solvent};
    \node[color = white] at (0,0) {\tiny solvent + oil + gas};
    \node[color = white] at (0,\x - 0.3) {\tiny gas + solvent};
