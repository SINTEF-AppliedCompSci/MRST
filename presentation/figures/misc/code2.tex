...
\begin{tikzpicture}[node distance = 2cm, auto]
  ...
  \begin{scope}[shift = {(0,0)}]
    ...
    \node [code, right = 3cm of init, anchor = north west] (initCode) {
      ...
      [sWres, sOres , sSGres] = computeResidualSaturations(fluid, p , sG , sS );\\\vspace{1em}
      % \% Calculate effective relperms
      [krW, krO, krG, krS] = computeRelPermSolvent(fluid, ...);\\\vspace{1em}

      % Calulate effective viscosities and densities
      [muW, muO, muG, muS, rhoW, rhoO , rhoG , rhoS ] ... \\
      \hspace{2cm} = computeViscositiesAndDensities(fluid, ...);\\\vspace{1em}

      % Conservation of mass for water
      water \hspace{2.2ex}= (op.pv/dt).*( pvMult.*bW.*sW - pvMult0.*bW0.*sW0 ) + op.Div(bWvW);

      % Conservation of mass for oil
      oil \hspace{4.7ex}= (op.pv/dt).*( pvMult.*bO.*sO - pvMult0.*bO0.*sO0 ) + op.Div(bOvO);

      % Conservation of mass for gas
      gas \hspace{3.9ex} = (op.pv/dt).*( pvMult.*bG.*sG - pvMult0.*bG0.*sG0 ) + op.Div(bGvG);
  ...
  \begin{scope}[shift = {(0,\y + \dy)}]
    \node [block2] (prop) at (0,0) {Compute properties and assemble equations};
  \end{scope}
...
% solvent = (op.pv/dt).*( pvMult.*bS.*sS - pvMult0.*bS0.*sS0 ) + op.Div(bSvS);};
  ...
  % Draw edges
  \path [line] (init) -- node[pos = 0.5, anchor = west]{$n = 0, \, m = 0$}(dtm);
  \path [line] (dtm) -- (prop);%
  ...
  \path [line2] (prop.east) -- (initCode.north west);
  \path [line2] (prop.east) -- (initCode.south west);
