...
\begin{frame}
  ...
  \only<1>{
    ...
    \item 10 WAG-cycles over 4 years, 1 PV injected in each cycle
    \end{itemize}
  }
  \only<2>{
    \begin{center}
      \begin{mybox}{Moderate mixing ($\omega \doteq 2/3$)}{0.63\textheight}
        \begin{center}
  ...
  \only<3>{
    \centering
    \begin{mybox}{Full mixing ($\omega \doteq 1$)}{0.63\textheight}
      \begin{center}
        \movie[showcontrols, width = 1\textwidth]{\includegraphics[width = \textwidth]{figures/placeholder}}
...
\begin{frame}
  ...
  \only<2>{
    \begin{itemize}
    \item Layer 10 of SPE10 model 2, initially filled with oil
...
\begin{frame}
  \frametitle{Example 2: Norne field model}
   \only<1>{
    ...
    \end{figure}
    \begin{itemize}
    \item Realistic field model with $44,000$ cells, initially filled with oil
