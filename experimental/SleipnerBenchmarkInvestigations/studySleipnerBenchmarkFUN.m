...
opt = merge_options(opt, varargin{:});
    ...
    fprintf('\nGetting grid...\n\n')
    if ~strcmpi(opt.gridname,'SEISMICmodel')
        [ G, Gt, rock, rock2D ] = makeSleipnerModelGrid('modelName', opt.gridname, 'refineLevel',opt.refineLevel);
    else
        [ G, Gt, rock, rock2D ] = makeSeismicModelGrid( opt.refineLevel );
    end
    fprintf('\n\n...grid obtained.\n\n')
    ...
    [ schedule, var_tmp ] = ...
        getInjRatesAndTimeSteps('ratecase'          , opt.ratecase, ...
                                'num_years'         , opt.num_years, ...
                                'num_steps_per_year', opt.num_steps_per_year);
    ...
    for i = 1:numel(var.inj_rates)
        schedule.control(i).W = addWell([], Gt.parent, rock2D, ...
            wellCellIndex, 'name', sprintf('W%i', 1), 'Type', 'rate', ... % can well fields have same name?
