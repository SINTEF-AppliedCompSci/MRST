...
%     %dTm         = mig_time / mig_steps; % timestep size in seconds
    ...
    %% Schedule: add discrete rates, time steps, well controls
...
%     istepvec                = repmat( ones(inj_steps, 1) * var.dTi , [numel(var.inj_rates) 1] );
%     %mstepvec                = ones(mig_steps, 1) * dTm;    
%     schedule.step.val       = var.dTi; %istepvec; %[istepvec; mstepvec];
%     schedule.step.control   = [ 1:1:numel(var.dTi) ]';
