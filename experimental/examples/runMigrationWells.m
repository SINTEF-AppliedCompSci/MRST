...
% are taken from the aforementioned paper. Hydrostatic boundary conditions
...
while t<T
   ...
   while t_loc < dTplot
       ...
       if(~isempty(w))       
        fprintf(1,'Maximum pressure well diff %f\n', convertTo(max(vertcat(sol.state.wellSol.bhp)),barsa));
       end
...
%         %fluidADI = addVERelperm(fluidADI,'res_water',sw,'res_gas',sr,'Gt',Gt);
...
%         fluidADI = addVERelperm_topmod(fluidADI,'res_water',sw,'res_gas',sr,'Gt',Gt,'top_trap',opt.top_trap);
