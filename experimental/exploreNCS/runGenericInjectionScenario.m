...
%   rock2D  - available rock properties (transmissibilities can be updated
%   using the transmissibility factor, transMult, if faultFaces are
%   specified)
...
%               - alternatively: wells can be pressure-controlled
...
%   exploreSimulation
    ...
    elseif isfield(wellinfo,'BHP')
        opt.initOverP = wellinfo.initOverP;
    end
    ...
    opt.faultFaces        = [];
    opt.wellType          = 'rate'; % can be set to 'bhp'
    ...
    if strcmpi(opt.wellType,'rate')
        allRates = [ opt.inj_rate; -opt.prod_rate ];
        assert(numel(allRates) == numWells);
        wellVal = allRates;
    elseif strcmpi(opt.wellType,'bhp')
        for i=1:numWells
            wcell_inx(i) = getCellIndex(Gt, allWells(i,1), allWells(i,2));
        end
        ...
        opt.BHP = initState.pressure(wcell_inx) + wellinfo.initOverP;
        injBHP  = opt.BHP; %ones(numWells,1) .* opt.BHP;
        assert(numel(injBHP) == numWells);
        wellVal = injBHP;
    else
    ...
    for i = 1:numWells
        ...
        if (strcmpi(opt.wellType,'rate') && allRates(i) > 0) ...
                || (strcmpi(opt.wellType,'bhp') && injBHP(i) > 0)
            % inj
