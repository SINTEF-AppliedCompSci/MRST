function [state, report] = implicitTransportVE(state, G, tf, ...
                                      rock, fluid, varargin)
...
% $Revision: 9020 $
   ...
   % ----------------------------------------------------------------------
   state_old=state;
   [F, Jac] = twophaseJacobianWithVE_coupled(G, state, rock, fluid, ...
      'wells', opt.wells, ...
      'src',   opt.src, ...
      'bc',    opt.bc, ...
      'vert_avrg',opt.vert_avrg, 'region3D', opt.region3D);
                            
   update = @(resSol, s0, ds, dt, err)        ...
              linesearch(resSol, ds, opt.resred * err,  ...
                         @(resSol) F(resSol, s0, dt),   ...
                         opt.lstrials,state_old);

   dispif(opt.verbose, '\nimplicitTransport:\n');
   dispif(opt.verbose, [repmat('-', [1, 70]), '\n']);
   opt.resSol_old=state_old;
...
function [resSol, res, alph, fail] = linesearch(resSol, ds, target, F, ni,resSol_old)
%
...
%
   ...
   while fail && (i < ni),
      ...
      sn.s_max =max(sn.s,resSol_old.s_max);
