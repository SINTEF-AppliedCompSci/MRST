function [ff, gg, hh, gp, dF, dC] = computePressureRHSVE_coupled(g, omegaCF, bc, src, varargin)
%Compute rhs contributions to pressure linear system, for coupled system.
%
% SYNOPSIS:
...
%
%   omegaCF - Accumulated phase densities \rho_i weighted by fractional flow
%           functions f_i -- i.e., omega = \sum_i \rho_i f_i.  One scalar
%           value for each cellFace in the discretised reservoir model, g.
%
...
%
%  'pn'/pv - List of 'key'/value pairs 
%          - region3D -- numel(region3D) = g.cells.num, 
%                        region3D = true for 3D cells. 
%
% RETURNS:
...
% $Revision: 9020 $
   opt = struct('region3D', false(g.cells.num,1));
   opt = merge_options(opt, varargin{:});
   
   gp = grav_pressure(g, omegaCF, opt.region3D);
   ff = zeros(size(gp));
...
function ff = grav_pressure(g, omegaCF, region3D)
...
g_vec = gravity();   % Must be a 1-by-3 row vector for subsequent code.
   ...
   if norm(g_vec) > 0,       
      ...
      % VE - 3D
      cellCentroid = g.cells.centroids(cellno, :);
      if any(region3D)
         cells2D = ~region3D;
         cellF2D = find(cells2D(cellno));
         % use centroid of underlying 3D cell for boundary faces
         cellCentroid(g.facesBnd.cellFace2D,:) = g.facesBnd.centroid3D;
      else
         cellF2D = true(size(g.cells.faces,1),1);
      end
      
      cvec   = g.faces.centroids(g.cells.faces(:,1), :) - cellCentroid;
      
      hftb = any(bsxfun(@eq,g.cells.faces(cellF2D,2),[5,6]),2);
      % remove contribution from top and bottom faces, but only for 2D (VE)
      % cells i.e. not for real 3D cells (in the coupled version)
      cvec(cellF2D(hftb),:) = 0;
      
      ff     = omegaCF .* (cvec * g_vec(1:dim).');
