...
for i=1:numel(names)
    ...
    for r = 1:numel(cp)
        ...
        'dryrun'                         , false                      , ... % if true, will not proceed to optimization
        ...
        'schedule'                       , [], ...%'/home/rebeccaa/Documents/MATLAB/mrst-bitbucket/Synthetic_noWellCost/leakage/cpLeak2/numWells_2/InjYrs30_MigYrs3000_resTrap0_IR0pt5/optim', ...%sch                          , ... % can also pass in path of a previously saved schedule
            'itime'                      , 8 * year                    , ...
            'isteps'                     , 8                           , ...
            'mtime'                      , 300 * year                  , ...
            'msteps'                     , 30                          , ... 
            'extended_mig_time'          , 100000 * year, ... % if sch is passed in, mig period can be extended
        ...
        'well_placement_type'            , 'one_per_path'               , ... % options: 'use_array', 'one_per_trap', 'one_per_path'
            'max_num_wells'              , 5                           , ... % used in use_array, one_per_trap
            'well_nums_to_keep'          , [4]                            , ... % ignore if empty. Could be scalar or array of well numbers
            ...
            'well_buffer_dist_domain'    , 0 * kilo * meter, ...%5 * kilo * meter             , ... % dist from edge of domain    
        ...
        'useBreakEvenRate'               , false                         , ... % break-even rate = well investment cost / (tax credit rate - operation rate);
        ...
        'account_well_cost'              , true                         , ...
            'well_initial_cost'          , 0, ...%7e6, ...%0, ...%1e6                          , ... % cost of having [$]
    ...
    if exist('other','var')
        subVarDirName = [fmName '_assessInjTimeFrame' '/' ...
            ...
            'wellNum_',num2str(other.opt.well_nums_to_keep), '/' ... %'numWells_',num2str(numel([optim.schedule.control(1).W.val])), '/' ...
