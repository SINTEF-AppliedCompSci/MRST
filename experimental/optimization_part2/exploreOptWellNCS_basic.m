...
% testing). Additionally, a penalty for well cost can account for initial
...
for i=1:numel(names)
    ...
    for r = 1:numel(cp)
        ...
        'lineSearchMaxIt'                , 5                           , ...
        'gradTol'                        , 1e-3                         , ...
        'objChangeTol'                   , 1e-3, ...%1e-4                         , ...
        'modelname'                      , fmName                       , ...
            'coarse_level'               , coarsening                   , ...
        'schedule'                       , [], ...%'/home/rebeccaa/Documents/MATLAB/mrst-bitbucket/Synthetic_assessWellCost_updatedObjFun/leakage/cpLeak10/WellInvestCost_0/numWells_8/InjYrs10_MigYrs300_resTrap0_IR0pt5/optim', ...%sch                          , ... % can also pass in path of a previously saved schedule
            'itime'                      , 10 * year                    , ...
            'isteps'                     , 10                           , ...
            'mtime'                      , 300 * year                  , ...
            'msteps'                     , 30                          , ... 
            'extended_mig_time'          , 300 * year, ... % if sch is passed in, mig period can be extended
            'set_mig_rates_to_0'         , true                         , ... % use true for a dry-run only
        'well_placement_type'            , 'one_per_trap'               , ... % options: 'use_array', 'one_per_trap', 'one_per_path'
            'max_num_wells'              , 10                           , ... % used in use_array, one_per_trap
            'well_nums_to_keep'          , 3                            , ... % ignore if empty. Could be scalar or array of well numbers
            ...
            'well_buffer_dist_domain'    , 1 * kilo * meter, ...%5 * kilo * meter             , ... % dist from edge of domain    
        ...
        'useBreakEvenRate'               , true                         , ... % break-even rate = well investment cost / (tax credit rate - operation rate);
        'well_control_type'              , 'rate'                       , ...
            'rate_lim_fac'               , max_rate_fac                 , ...
            'max_wvals'                  , max_wvals                    , ...
        'btype'                          , 'pressure'                   , ... % options: 'pressure', 'flux'
        'penalize_type'                  , 'leakage_at_infinity'                   , ... % options: 'pressure', 'leakage', 'leakage_at_infinity'
            'leak_penalty'               , 1                          , ...
            'pressure_penalty'           , cp(r)                        , ... % appropriate penalty is iteratively determined for closed-systems
            'p_lim_factor'               , 0.9                          , ...
        'account_well_cost'              , true                         , ...
            'well_initial_cost'          , 7e6, ...%0, ...%1e6                          , ... % cost of having [$]
            'well_operation_cost'        , 40                           , ... % cost of operating [$/tonne], between 40-60 $/tonne
            'co2_tax_credit'             , 51                           , ... % [$/tonne] i.e., average 21 $/tonne, as high as 51 $/tonne
            'nonlinear_well_cost'        , false                        , ...  % if true, opt.alpha must not be empty
            'alpha'                      , 0.5                            , ... % i.e., tanh(q/(alpha*q_crit))
    ...
    if exist('other','var')
        subVarDirName = [fmName '_assessWellCost_updatedObjFun' '/' ...
            other.opt.penalize_type '/' ...
            'cpLeak',num2str(other.opt.leak_penalty), '/' ...
            'WellInvestCost_',num2str(other.opt.well_initial_cost), '/' ... %'alpha_',num2str(other.opt.alpha), '/' ...
            'numWells_',num2str(numel([optim.schedule.control(1).W.val])), '/' ...
            'InjYrs',num2str(convertTo(other.opt.itime,year)), ...
            '_MigYrs',num2str(convertTo(other.opt.mtime,year)), ...
            '_resTrap',num2str(other.opt.sr), ...
            '_IR0pt5'];
        mkdir(subVarDirName);
