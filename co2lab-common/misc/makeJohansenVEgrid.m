...
% ensure dataset has been downloaded
[dataset_info, is_present] = dataset_johansen(); % from mrst-core
jdir = fullfile(mrstDataDirectory(), dataset_info.name);
if ~is_present
    % dataset must be downoaded first
    [jdir, success] = downloadDataset(dataset_info.name, true);
    if ~success
        error('Johansen dataset not downloaded.  Aborting.');
    end
end
