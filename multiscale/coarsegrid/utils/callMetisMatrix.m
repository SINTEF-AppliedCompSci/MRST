...
%}
   ...
   if ~isempty(METISPATH) && isdir(METISPATH) && ...
       ...
       if ~ st
        ...
        fprintf(['Please make sure METIS is installed and the METISPATH is set in ' ...
            'your startup_user.m file.\n\n' ...
            'Linux users: install via package manager (e.g. apt) and add the following to startup_user.m:\n' ...
            'global METISPATH; METISPATH = fullfile(''/usr'',''local'',''bin'')\n\n' ...
            'Mac users: install via package manager (e.g. homebrew) and add the following to startup_user.m:\n' ...
            'global METISPATH; METISPATH = fullfile(''/opt'',''homebrew'',''bin'')\n\n' ...
            'If you need to build METIS from source, you can download it from \n' ...
            'https://github.com/KarypisLab/METIS and follow the instructions in the README file.\n\n']);
         error(msgid('METIS:NotFound'), ...
                 'METIS binary ''%s'' not found in search path.', binname);
    end
