function [model_out,W,indexs] = createDDmodel(model,nc,Graph,wells,levels)
 ...
 for i = 1:num_edges
     if if_well(i)==1
         ...
         W = verticalWell(W, model.G, model.rock, model.G.cartDims(1), model.G.cartDims(2), k_levels, ...      
             'Type', wells.type{Well_p} , 'Val', wells.val{Well_p}, ... %  SAIGUP model
            'Radius', 0.1,'Name',wells.name{Well_p},'Comp_i', [0.5, 0.5],'sign', -1);
                    %'Type', 'bhp' , 'Val', 395*barsa, ... %  Egg model 

         producer_cell = W(end).cells;
         fprintf('\n\nWe are at well producer %d - %s  with well cell [ ', Well_p, wells.name{Well_p})
         ...
         for j =1:length(Well_I)
              fprintf('\n+++ We are at conection  %s - %s \n', wells.name{Well_I(j)},  wells.name{Well_p});
            ...
            % Ask if we need an injector   
             if if_inj_well(Well_I(j))==1  % if Yes Make injector
                 fprintf('\n+++ Create injector and conectionr for well %s  - %s\n', wells.name{Well_I(j)},wells.name{Well_I(j)})
                 fprintf('\n+++ At level %d - %d \n', k_levels(1),k_levels(2))

                 % TODO ADD levels here
                 W = verticalWell(W, model.G, model.rock, 1, 1, k_levels, ...       
                                    'Type', wells.type{Well_I(j)} , 'Val',  wells.val{Well_I(j)}, ... %   
                                    'Radius', 0.1,'Name',wells.name{Well_I(j)},'Comp_i', [1, 0],'sign',1);
