function parameter_struct = brooks_corey_pc_handle(model, x, row_names)
%
...
%
% SYNOPSIS:
%   parameter_struct = brooks_corey_pc_handle(model, x, row_names)
%
% PARAMETERS:
...
%handle pd
pd_mask = strcmpi(row_names, 'pd');
if any(pd_mask)
    parameter_struct.pd = x(pd_mask);
else
    if isfield(pc_input_struct, 'pd')
        parameter_struct.pd = pc_input_struct.pd;
    else
        parameter_struct.pd = 0.005;
    end
end
