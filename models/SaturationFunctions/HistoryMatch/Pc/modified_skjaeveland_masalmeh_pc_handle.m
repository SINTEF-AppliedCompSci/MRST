function parameter_struct = modified_skjaeveland_masalmeh_pc_handle(model, x, row_names)
%
...
%
% SYNOPSIS:
%   parameter_struct = modified_skjaeveland_masalmeh_pc_handle(model, x, row_names)
%
% PARAMETERS:
...
%handle sw_cutoff
sw_cutoff_mask = strcmpi(row_names, 'sw_cutoff');
if any(sw_cutoff_mask)
    parameter_struct.sw_cutoff = x(sw_cutoff_mask);
else
    if isfield(pc_input_struct, 'sw_cutoff')
        parameter_struct.sw_cutoff = pc_input_struct.sw_cutoff;
    else
        parameter_struct.sw_cutoff = 0.5;
    end
end

%handle b
b_mask = strcmpi(row_names, 'b');
if any(b_mask)
    parameter_struct.b = x(b_mask);
else
    if isfield(pc_input_struct, 'b')
        parameter_struct.b = pc_input_struct.b;
    else
        parameter_struct.b = 0.001;
    end
end