...
so=1-sw-sg;
if(true)
 so=max(epsilon,so);min(1,so);
 sw=max(0,sw);min(1,sw);
 sg=max(0,sg);min(1,sg);
 assert(all((so+sw+sg)>0))
 state.s  = bsxfun(@rdivide,[sw, so, sg],(so+sw+sg));
else
    state.s  = [sw, 1-sw-sg, sg];
end
state.rs = min(rs,rsSat);
state.rs(state.s(:,3)>0)=rsSat(state.s(:,3)>0);
state.rs(state.s(:,1)==1)=rsSat(state.s(:,1)==1);
state.rs(state.s(:,2) < sqrt(eps)) = rsSat(state.s(:,2) < sqrt(eps));
assert(all(isfinite(state.s(:))));
assert(all(isfinite(state.pressure(:))));
%/;
