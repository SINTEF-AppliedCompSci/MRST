...
coordind = 1;
dirface = G.faces.centroids(:, coordind);
maxdir = max(dirface);
mindir = min(dirface);
maxfaces = find(dirface > (maxdir - 1e-4));
minfaces = find(dirface < (mindir + 1e-4));
W = [];
bc = [];
bc = pside(bc, G, 'Xmin', 10, 'sat', [0, 1]);
bc = pside(bc, G, 'Xmax', 20, 'sat', [0, 1]);
...
%interpFace = [];
%OSflux = [];
%model_ntpfa = setNTPFADiscretization(model, 'myRatio', ratio, 'interpFace', interpFace, 'OSflux', OSflux);
model_ntpfa = setNTPFADiscretization(model, 'saveInterpFaceDir', '/tmp', 'saveOSfluxDir', '/tmp');
[wsNTPFA, statesNTPFA] = simulateScheduleAD(state0, model_ntpfa, schedule);
plotter(G, statesNTPFA, 'NTPFA')
...
>>>>>>> 850841b5864cba77e063fb3ea4a8d3e2a3a6fbdb
