...
%  multiplied by the indicator function, as it appears in the paper. 
    ...
    switch d 
      ...
      case 3
        ...
        f{1} = @(x, y, z) mu0*(16*x.^2.*y.^2 - 16*x.^2.*y + 16*x.^2.*z.^2 - 16*x.^2.*z + 8*x.^2 - 16*x.*y.^2 + 16*x.*y - 16*x.*z.^2 + 16*x.*z - 8*x + 16*y.^2.*z.^2 - 16*y.^2.*z + 8*y.^2 - 16*y.*z.^2 + 16*y.*z - 8*y + 8*z.^2 - 8*z + 3)/8;
        f{2} = @(x, y, z)  mu0*(16*x.^2.*y.^2 - 16*x.^2.*y + 16*x.^2.*z.^2 - 16*x.^2.*z + 8*x.^2 - 16*x.*y.^2 + 16*x.*y - 16*x.*z.^2 + 16*x.*z - 8*x + 16*y.^2.*z.^2 - 16*y.^2.*z + 8*y.^2 - 16*y.*z.^2 + 16*y.*z - 8*y + 8*z.^2 - 8*z + 3)/8;
        f{3} = @(x, y, z)  (2*z - 1).*(3*lamb0*(2*x - 1).^2.*(2*y - 1) + 3*lamb0*(2*x - 1).*(2*y - 1).^2 - 6*lamb0*(2*x - 1).*(2*y - 1).*(x + y - 1) - 12*mu0*(2*x - 1).*(2*y - 1).*(x + y - 1) + mu0*((1 - 2*x).*(2*z - 1).^2 + (2*x - 1).^2.*(6*y - 3)) + mu0*((1 - 2*y).*(2*z - 1).^2 + (6*x - 3).*(2*y - 1).^2))/12;
