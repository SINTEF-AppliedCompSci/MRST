...
%  multiplied by the indicator function, as it appears in the paper. 
    ...
    switch d 
      case 2
        ...
        f{1} = @(x, y) 0.5*(2*x - 1).^2 + 0.5*(2*y - 1).^2;
        f{2} = @(x, y) -4.0*x.*y + 2*x + 2*y - 1;
      ...
      case 3
        ...
        f{1} = @(x, y, z) (-0.5*y.^2.*(2*z - 1).^2 + 0.5*y.*(2*z - 1).^2 + ...
                           0.125*(2*x - 1).^2.*(2*y - 1).^2 + 0.125*(2*x - ...
                                                          1).^2.*(2*z - 1).^2 ...
                           + (2*y - 1).^2.*(2*z - 1).^2/4 - 0.125*(2*z - 1).^2);
        
        f{2} = @(x, y, z) (-0.5*x.^2.*(2*z - 1).^2 + 0.5*x.*(2*z - 1).^2 + ...
                           0.125*(2*x - 1).^2.*(2*y - 1).^2 + (2*x - 1).^2.*(2*z ...
                                                          - 1).^2/4 + 0.125*(2*y ...
                                                          - 1).^2.*(2*z - 1).^2 ...
                           - 0.125*(2*z - 1).^2);
        
        f{3} = @(x, y, z) ((1 - 2*x).*(2*z - 1).^3/12 + (1 - 2*y).*(2*z - ...
                                                          1).^3/12 + (2*x - ...
                                                          1).*(2*y - 1).^ ...
                           2.*(2*z - 1)/4 + 0.25*(2*x - 1).^2.*(2*y - 1).*(2*z ...
                                                          - 1) - (2*x - 1).*(2*y ...
                                                          - 1).*(12*z - 6).*(x ...
                                                          + y - 1)/6);
