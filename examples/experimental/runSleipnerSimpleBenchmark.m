function [Gt,sol, sreport]=runSleipnerSimpleBenchmark(method,case_name,varargin)
...
%% Display header
%clc
opt = struct('Verbose', mrstVerbose,'plot',true,'T',[],'rhow_eff',1200,'mod_rock',false,'sim',[]);
...
[transport_solver fluidVE_h fluidVE_s, fluidADI]=...
    makeTransportSolver(transport_method,Gt,rock, rock2D,cpp_accel,T_ref);
...
if isempty(opt.sim),
 sim=struct('total_time',100*year,...
    'stopInject',  30*year(),...
    'start_dt',  0.05*year(),...
    'inj_max_dt', 1*year(),...
    'post_min_dt', 0.1*year(),...
    'post_max_dt', 5*year(),...
    'inj_plot_dt',1*year());
else
    sim=opt.sim;
...
T          = sim.total_time
stopInject = sim.stopInject;
dT         = sim.start_dt;
dT_max     = sim.inj_max_dt;
dTpost_min = sim.post_min_dt;
dTpost_max = sim.post_max_dt;% also plotting timestep for post injection
dTplot     = sim.inj_plot_dt;
...
z_meanB=mean(Gt.faces.z);
% BC in 2D model
bcVE = addBC([], bcIxVE, 'pressure', ...
            opt.rhow_eff*z_meanB*norm(gravity())+...
            (Gt.faces.z(bcIxVE)-z_meanB)*fluidVE_h.rho(2)*norm(gravity));
...
function [transport_solver fluidVE_h fluidVE_s fluidADI]= makeTransportSolver(solver,Gt,rock, rock2D, cpp_accel,T_ref)    
    ...
    if(true)
        ...
        %fluidADI.bG  =  boCO2(T, fluidADI.rhoG);
