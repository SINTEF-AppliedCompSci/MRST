...
plotWellSols({wellSolPred, wellSolTrain}, ...
...
objh = @(p) evaluateMatch(p,mismatchFn,cTrainProbl,trainPrms,statesTrain);
...
[misfitP,~,wellSolP] = ...
    evaluateMatch(popt,mismatchFn,cPredProbl,predPrms,statesPred,'Gradient','none');
[misfitT,~,wellSolT] = ...
    evaluateMatch(popt,mismatchFn,cTrainProbl,trainPrms,statesTrain,'Gradient','none');

%% Plot well curves
fh = plotWellSols({wellSolTrain,wellSolT}, ...
...
%%
fh = plotWellSols({wellSolPred,wellSolP}, ...
