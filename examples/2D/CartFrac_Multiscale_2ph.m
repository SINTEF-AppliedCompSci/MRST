...
min_size = 1; cell_size = 5; plotperm = 0;
...
figure; plotFracData2D(G, IRf,'wide',true,'F',F,'CG',CGf,'plotMatrix',true,'cmap',flipud(winter));
...
bc = [];
bc  = pside(bc, G, 'LEFT', 10*barsa(),'sat',[1,0]);
bc  = pside(bc, G, 'RIGHT', 1*barsa(),'sat',[0,1]);
wellRadius = 1e-2;
W = addWell([], G.Matrix, G.Matrix.rock, 1,'InnerProduct', 'ip_tpf','Type', ...
    'rate', 'Val', 1.0/day(),'Radius', wellRadius, 'Comp_i', [1, 0]);
W = addWell(W, G.Matrix, G.Matrix.rock, G.Matrix.cells.num, 'InnerProduct', 'ip_tpf', 'Type', ...
    'bhp' , 'Val', 1.0*barsa(), 'Radius', wellRadius, 'Comp_i', [0, 1]);
...
figure; plotFracData2D(G, basis_sb.B(G.Matrix.cells.num+1:G.cells.num,dof_matrix+1:max(CG.partition)), ...
...
%             Gfl = G.FracGrid.(['Plane',num2str(i)]);
