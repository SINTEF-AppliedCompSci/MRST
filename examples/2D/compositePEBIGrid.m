...
fl = zeros(numel(lines),4);
for i = 1:numel(lines)
    fl(i,:) = [lines{1,i}(1,:) lines{1,i}(2,:)];
end
nx = 25; ny = 25; lx = 1000; ly = 1000;
G = compositeGridPEBI([nx, ny], [lx, ly], 'makePEBI', true);
G.nodes.coords = round(G.nodes.coords);
G = computeGeometry(G);
figure;
plotGrid(G)
...
% T = T(1:G.Matrix.faces.num);
% T(G.faces.tag>0) = 0;

[CG, CGf] = getRsbGrids_HFM(G, fracture.network, 'coarseDims', coarseDims, ...
    'use_metis', true, 'dof_matrix', dof_matrix, 'dof_frac', dof_frac,...
    'sysMatrix', A, 'MatrixTrans',T(1:G.Matrix.faces.num));

dof_matrix = max(CG.partition(1:G.Matrix.cells.num));
figure; plotFractureCoarseGrid2D(G,CG.partition,F)
figure; outlineCoarseGrid(CG.parent.Matrix,CG.partition(1:G.Matrix.cells.num));
...
bc = [];
wellRadius = 0.1;
wcells = findQuarter5spotLocations(G,[10 10],[990 990]);
W = addWell([], G.Matrix, G.Matrix.rock, wcells(1),'InnerProduct', 'ip_tpf','Type', ...
    'rate', 'Val', 1000/day(),'Radius', wellRadius, 'Comp_i', [1, 0]);
W = addWell(W, G.Matrix, G.Matrix.rock, wcells(2), 'InnerProduct', 'ip_tpf', 'Type', ...
    'bhp' , 'Val', 200*barsa(), 'Radius', wellRadius, 'Comp_i', [0, 1]);
