%% This is an EDFM model of the Eagle Ford case with 7 fracture stages.
% It is very computationally expensive because it is 28 times bigger
% than the stencil discussed in the chapter write-up.
...
heelCooord=[130, physdim(2)/2, physdim(3)/2];
ToeCoord=[heelCooord(1)+wellLength,physdim(2)/2, physdim(3)/2];
BHP=1000*psia;
fracCentroid1=[130, physdim(2)/2, physdim(3)/2];
...
[fracplanes,frac_centroid_s] = createMultiStageHFs('numStages',NumStages,'fracSpacing', fractureSpacing,...
      ...
      'clusterSpacing', clusterSpacing,'fracCentroid1',fracCentroid1,'isStencil',0);
...
try
    %% This code requires ADFNE and the Statistics & Machine Learning Toolbox
    set1 = Field(DFN('dim',3,'n',2100,'dir',45,'ddir',-100,'minl',5,'mu',20, ...
        'maxl',20,'bbx',[tol,tol,tol,physdim(1)-tol, ...
        physdim(2)-tol,physdim(3)-tol],'dip',45,'ddip',-100, ...
        'shape','l','q',4),'Poly');
    set2 = Field(DFN('dim',3,'n',2100,'dir',-45,'ddir',-100,'minl',5,'mu',20, ...
        'maxl',20,'bbx',[tol,tol,tol,physdim(1)-tol, ...
        physdim(2)-tol,physdim(3)-tol],'dip',45,'ddip',-100, ...
        'shape','l','q',4),'Poly');
catch
    %% Load generated fractures when ADFNE is not available
    load('EagleFordfullFracs.mat');
end
...
%% EDFM PreProcessing
% The preprocessing takes a while. The user can  remove line 145 to 155 and
% instead load the preprocessed grid at line 158
...
% load('EagleFordEDFMfullGrid.mat');
