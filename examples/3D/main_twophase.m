...
W   = addWell([], G.Matrix, G.Matrix.rock, cellinj, 'Type', 'rate',...
    'Val', 1000/day, 'Radius', radius, 'Sign',1, 'Comp_i', [1, 0]);
...
figure; plotToolbar(G,sol_fs); plotWell(G,W); view(-20,20); colormap(flipud(jet)); caxis([0 1]);
figure; plotToolbar(G,sol_ms); plotWell(G,W); view(-20,20); colormap(flipud(jet)); caxis([0 1]);
figure; plotToolbar(CGf.parent,basis_frac)
plotWellSols({wellSol_fs,wellSol_ms},pvi); 
figure; plot(pvi,L1)
