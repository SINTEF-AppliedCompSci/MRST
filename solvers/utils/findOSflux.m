...
% Construct one-side fluxes for 2D and 3D grids. Considering general boundary
% conditions, appending a constant at the last row of transmissibility matrix
% Dirichlet boundary faces are treated as zero volume cells to derive nonlinear
% two-point flux approximation for Dirichlet boundary faces
        ...
        for i_face = 1 : G.faces.num
            ...
            if (all(G.faces.neighbors(i_face, :) ~= 0)) %------------------------------internal face
                ...
                if (strcmpi(bc.type{ind}, 'pressure'))
                    ...
                    if (c1 ~= G.faces.neighbors(i_face, 1))
                        fn = -fn;
                    end
                    w1 = K1 * fn;
                    [a, faceA, faceB] = findAB(G, interpFace, c1, w1);
        ...
        for i_face = 1:G.faces.num
            ...
            if (all(G.faces.neighbors(i_face, :) ~= 0)) %--------------internal face
                ...
                if (strcmpi(bc.type{ind}, 'pressure'))
                    ...
                    [a, faceA, faceB, faceC] = findABC(G, interpFace, c1, w1);
                    if (faceA == i_face)
                        interpB = G.faces.neighbors(faceB, :)';
                        weightB = -a(2) .* interpFace.weights(faceB, :)';
                        interpC = G.faces.neighbors(faceC, :)';
                        weightC = -a(3) .* interpFace.weights(faceC, :)';
                        container = [c1; cf; interpB; interpC; 0];
                        container(:, 2) = [sum(a); -a(1); weightB; weightC; 0];
                    elseif (faceB == i_face)
                        interpA = G.faces.neighbors(faceA, :)';
                        weightA = -a(1) .* interpFace.weights(faceA, :)';
                        interpC = G.faces.neighbors(faceC, :)';
                        weightC = -a(3) .* interpFace.weights(faceC, :)';
...
function [a, faceA, faceB] = findAB(G, interpFace, c, Kn)
    ...
    assert(logical(exist('faceA', 'var')), ['decomposition failed for cell ', ...
                        num2str(c)]);
