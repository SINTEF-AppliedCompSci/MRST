function varargout = mex_opmtransport_implicit(varargin)
...
%}
   ...
   INCLUDE = { '-I/usr/include/suitesparse'                          , ...
              ['-I', fullfile(d, '..', 'include', 'opm', 'transport')], ...
   ...
   SRC = { 'mex_opmtransport_implicit.c'                           , ...
           fullfile(d, '..', 'mrst_api', 'mrst_api.c'), ...
           fullfile(d, '..', 'mrst_api', 'call_umfpack.c') };

   LIBS = { '-lopmtransport', '-lmwumfpack', '-lmwamd', ...
            '-lmwlapack', '-lmwblas' };
   ...
   % Call MEX'ed edition.
   [varargout{1:nargout}] = mex_opmtransport_implicit(varargin{:});
end
