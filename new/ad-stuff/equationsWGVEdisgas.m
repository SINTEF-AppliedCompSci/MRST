function [problem, state] = equationsWGVEdisgas(model, state0, state, dt, drivingForces, varargin)
   ...
   eqs_rs = addFluxesFromSourcesAndBC(model,...
            eqs_rs, {pW, 0}, {rhoW, 0}, {mobW, 0}, {rs .* bW, 0}, {sW, 0}, drivingForces); 
...
function eqs = compute_dissolution_equations(model, Gt, f, sG, sG0, sGmax, ...
   ...
   if f.dis_rate == 0
      ...
      % Taking boundary conditions and sources into account (@@ not tested)
      eqs_rs = {eqs{1} * 0, eqs{1} * 0};
      eqs_rs = addFluxesFromSourcesAndBC(model, eqs_rs, {p, 0}, {rhoW, 0}, ...
                                         {mobW, 0}, {rs .* bW, 0}, {sW, 0}, ...
                                         drivingForces);
      ...
      if any(ix)
         eqs{2}(ix) = tmp(ix) - rate(ix) + (rs(ix) - rx0(ix));
      end
   end
