...
case 'ProdBot' 
    % Producer
%     W = addWell(W, G, G.rock, sub2ind(G.cartDims,20,10,frac_z(2)), ...
%         'comp_i', [0.23, 0.76, 0.01],'Name', 'Prod_Bot', 'Val', pwf, 'sign', -1,...
...
% qO2 = zeros(length(ws),1);
% wellCell = sub2ind(G_matrix.cartDims,20,10,frac_z(2));
% for i=1:length(ws)
% %     qInjRate(i) = ws{i}(2).qGs; %m^3/s
%     qO2(i) = ws{i}(1).qOr/stb; %rb/s
%     CumMassProd1(i)=InitialMass-sum(states{i}.rho(:,2).*G.cells.volumes.*G.rock.poro.*states{i}.s(:,2));%total mass produced in Kg
%     CumMassProd1(i)=CumMassProd1(i)/stb/states{i}.rho(wellCell,2);
% end
...
% 
% CorrectedORate = CumMassProd1/rhoO_S/stb; %in stb=>Uses dens of reservoir fluid at SC.
% ORate = CumMassProd2/rhoO_S/stb; %in stb=>Uses dens of reservoir fluid at SC.
% figure,
% plot(tinYears,CumMassProd1,tinYears,-cumtrapz(tinSecs,qO2))
% legend('CorrectedORate','ORate')
% 
