function Gf = gridPlanarFracture_mod(G_matrix, fracplane, scaledplane, varargin)
...
while true && opt.gridType~=1
    ...
    try
        ...
        close gcf
        Gf = triangleGrid(p,t);
        if opt.gridType==3, Gf = pebi(Gf); end % convert to PEBI if gridtype==3
...
dims = max(G_matrix.nodes.coords);
